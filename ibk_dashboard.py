...
const I18N={
  ...
  uzc:{archive:"Arxiv",upload:"Fayl yuklash",general:"Umumiy",companies:"Korxonalar",expired:"Muddati o'tgan",released:"Nazoratdan yechish",goods:"Tovarlar",food:"Oziq-ovqat",profile:"Profil",settings:"Sozlamalar",admin:"Admin",dark:"Tungi rejim",logout:"Chiqish",regimes:"Rejimlar",warehouses:"Omborlar",deadlines:"Muddatlar",validity:"Yaroqlilik",pay_overview:"Umumiy",pay_lists:"Hosil bo'lgan jadvallar",pay_analysis:"Tahlil"},
...
function renderKpis(){let k=DATA.kpis||{};
  ...
  let rd=DATA.meta&&DATA.meta.date||DATA.date||null;
  let dateCard=rd?`<div class=kpi style="background:${DATA_IS_STALE?'linear-gradient(135deg,#fef9c3,#fef3c7)':'linear-gradient(135deg,#f0f9ff,#dbeafe)'};border:1px solid ${DATA_IS_STALE?'#f59e0b':'#93c5fd'};cursor:default" title="${DATA_IS_STALE?'Bugungi ma\'lumot yuklanmagan':'Joriy ma\'lumot holati'}"><span style="font-size:11px;color:${DATA_IS_STALE?'#92400e':'#1d4ed8'}">📅 ${DATA_IS_STALE?'Oxirgi yuklangan':'Joriy holat'}</span><b style="color:${DATA_IS_STALE?'#b45309':'#1e40af'};font-size:15px">${rd}</b></div>`:'';
  return rows.map(x=>`<div class=kpi onclick="showKpi('${x[2]}')"><span>${x[0]}</span><b>${x[1]}</b></div>`).join("")+depHtml+aviaQiymatHtml+aviaHtml+dateCard;}
