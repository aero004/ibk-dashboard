  ...
  function draw(){
    ...
    for(var i=0;i<cells.length;i++){
      ...
      ctx.fillStyle=d?('rgba(59,130,246,'+(v*.38).toFixed(3)+')')
                     :('rgba(120,170,225,'+(v*.05).toFixed(3)+')');
      ctx.fill();
      ctx.strokeStyle=d?('rgba(96,165,250,'+(.28+v*.55).toFixed(3)+')')
                       :('rgba(120,170,225,'+(.05+v*.10).toFixed(3)+')');
