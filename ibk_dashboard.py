...
def build_dashboard(report_id: str, source: Path, deposit: Path | None, report_date: datetime) -> dict:
    ...
    for row in core.food_rows(df):
        item = {"key": {"tnved": core.to_latin(core.clean(row[1]))}, "name": core.to_latin(core.clean(row[1])), "vazn": float(row[2]), "qiymat": float(row[3]), "over_vazn": float(row[4]), "over_qiymat": float(row[5]), "ulush": 0.0}
    ...
    return {
        ...
        "kpis": {"partiya": int(df["_partiya"].sum()), "vazn": float(df["_weight_tn"].sum()), "qiymat": float(df["_value_usd_k"].sum()), "tolov": float(df["_pay_total_mln"].sum()), "depozit": float(deposit_total), "expired": int(expired["_partiya"].sum()), "expired_value": float(expired["_value_usd_k"].sum()) if len(expired) else 0.0},
...
.leaflet-popup-content{font:13px/1.4 "Segoe UI",Arial,sans-serif;color:#172033}
...
function renderKpis(){let k=DATA.kpis||{}, rows=[["Partiya",fmtI(k.partiya),"partiya"],["Vazn (tn)",fmtN(k.vazn),"vazn"],["Qiymat (ming $)",fmtN(k.qiymat),"qiymat"],["Kutilayotgan to'lov (mln so'm)",fmtN(k.tolov),"tolov"],["Depozit (mln so'm)",fmtN(k.depozit),"depozit"],["Muddati o'tgan partiya",fmtI(k.expired),"expired"]];return rows.map(x=>`<div class=kpi onclick="showKpi('${x[2]}')"><span>${x[0]}</span><b>${x[1]}</b></div>`).join("")}
...
const companyCols=[{k:"korxona",t:"Korxona nomi",w:"48%"},{k:"stir",t:"STIR",w:"10%"},{k:"partiya",t:"Partiya",n:1,f:fmtI},{k:"vazn",t:"Vazn (tn)",n:1,f:fmtN},{k:"qiymat",t:"Qiymat (ming $)",n:1,f:fmtN},{k:"tolov",t:"To'lov (mln so'm)",n:1,f:fmtN},{k:"depozit",t:"Depozit (mln so'm)",n:1,f:fmtN}], sumCols=[{k:"name",t:"Ko'rsatkich",w:"38%"},{k:"partiya",t:"Partiya",n:1,f:fmtI},{k:"vazn",t:"Vazn (tn)",n:1,f:fmtN},{k:"qiymat",t:"Qiymat (ming $)",n:1,f:fmtN},{k:"tolov",t:"Kutilayotgan to'lov (mln so'm)",n:1,f:fmtN}];
...
if(TAB==="goods"){let goodsCols=[{k:"name",t:"Tovar guruhi",w:"36%"},{k:"partiya",t:"Partiya",n:1,f:fmtI},{k:"korxona",t:"Korxona",n:1,f:fmtI},{k:"vazn",t:"Vazn (tn)",n:1,f:fmtN},{k:"qiymat",t:"Qiymat (ming $)",n:1,f:fmtN},{k:"tolov",t:"To'lov (mln so'm)",n:1,f:fmtN}], gp=topNWithOther(DATA.goods||[],30,"qiymat","name"), gpart=topNWithOther(DATA.goods||[],30,"partiya","name"), gvazn=topNWithOther(DATA.goods||[],30,"vazn","name");v.innerHTML=`<div class=grid2><div class="panel wide"><h2>Tovarlar guruhlari ${xls("goods")}</h2>${table(goodsCols,basicTotal(gp))}</div><div class=panel><h2>Partiya bo'yicha ulushi</h2>${bars(gpart,"name","partiya",fmtI)}</div><div class=panel><h2>Qiymat bo'yicha ulushi</h2>${bars(gp,"name","qiymat",fmtN)}</div><div class=panel><h2>Vazn bo'yicha ulushi</h2>${bars(gvazn,"name","vazn",fmtN)}</div></div><div class="panel" id="goodsTrendPanel"><h3>Tovarlar bo'yicha davriy tendensiya</h3><div class="muted">Yuklanmoqda...</div></div>`;loadGoodsTrends();return}
...
async function loadCompanyTrends(){let el=$("trendPanel");if(!el)return;try{let j=await api("/api/company_trends");COMPANY_TRENDS=j;let periods=j.periods||[],companies=j.companies||[];if(!periods.length){el.innerHTML=`<h3>Korxonalar bo'yicha davriy tendensiya</h3><div class="muted">Hali tarixiy ma'lumot yetarli emas. Har bir yangi yuklangan fayl bilan tendensiya shakllanadi.</div>`;return}let b=trendBuckets(periods,companies);let totalValues=periods.map((p,i)=>companies.reduce((a,c)=>a+(c.series[i].value||0),0));let totalWeights=periods.map((p,i)=>companies.reduce((a,c)=>a+(c.series[i].weight||0),0));el.innerHTML=`<h3>Korxonalar bo'yicha davriy tendensiya</h3><div class="overview-note">${periodRangeCaption(periods)}.</div><div class="kpis"><div class="kpi"><span>Jami korxonalar</span><b>${fmtI(companies.length)}</b></div><div class="kpi"><span>So'nggi davrda faol</span><b>${fmtI(b.active.length)}</b></div><div class="kpi"><span>Butunlay to'xtagan</span><b>${fmtI(b.stoppedAll.length)}</b></div><div class="kpi"><span>Yangi qo'shilgan</span><b>${fmtI(b.newAll.length)}</b></div></div><div class="panel"><h3>Umumiy oborot tendensiyasi (barcha korxonalar, ${periods[0]} - ${periods[periods.length-1]})</h3>${svgLineChart(periods,[{label:"Qiymat (ming $)",values:totalValues},{label:"Vazn (tn)",values:totalWeights}])}<div class="overview-note">Har qaysi korxona qatoriga bosilganda shu korxonaning alohida qiymat/vazn tendensiyasi ochiladi.</div></div><div class="grid2">${companyTrendTable(periods,b.stoppedAll,"Yuk olib kelishni butunlay to'xtatgan korxonalar (oldin faol bo'lgan, so'nggi davrda yo'q)","Barcha korxonalar faol","stopped")}${companyTrendTable(periods,b.newAll,"Yangi qo'shilgan korxonalar (so'nggi davrda birinchi marta faol)","Yangi korxona aniqlanmadi","new")}${companyTrendTable(periods,b.stopped2,"Oxirgi 2 davrda faolligini butunlay to'xtatgan korxonalar","Yo'q","stopped")}${companyTrendTable(periods,b.new2,"Oxirgi 2 davrda yangi faollashgan korxonalar","Yo'q","new")}</div>`}catch(e){el.innerHTML=`<h3>Korxonalar bo'yicha davriy tendensiya</h3><div class="muted">Ma'lumotni yuklab bo'lmadi: ${esc(e.message||e)}</div>`}}
function showCompanyTrend(stir){let c=(COMPANY_TRENDS.companies||[]).find(x=>x.stir===stir);if(!c)return;let periods=COMPANY_TRENDS.periods||[];let values=c.series.map(s=>s.value),weights=c.series.map(s=>s.weight);dlgTitle.textContent=`${c.company||c.stir} - davriy tendensiya (STIR: ${c.stir})`;let rows=c.series.map((s,i)=>({date:periods[i],value:s.value,weight:s.weight,partiya:s.partiya,key:{}}));dlgBody.innerHTML=`<div class="overview-note">${periodRangeCaption(periods)}.</div>`+svgLineChart(periods,[{label:"Qiymat (ming $)",values},{label:"Vazn (tn)",values:weights}])+table([{k:"date",t:"Davr",w:"110px"},{k:"value",t:"Qiymat (ming $)",n:1,f:fmtN},{k:"weight",t:"Vazn (tn)",n:1,f:fmtN},{k:"partiya",t:"Partiya",n:1,f:fmtI}],rows,"fixed-table");dlg.showModal()}
...
async function loadGoodsTrends(){let el=$("goodsTrendPanel");if(!el)return;try{let j=await api("/api/goods_trends");GOODS_TRENDS=j;let periods=j.periods||[],items=j.goods||[];if(!periods.length){el.innerHTML=`<h3>Tovarlar bo'yicha davriy tendensiya</h3><div class="muted">Hali tarixiy ma'lumot yetarli emas. Har bir yangi yuklangan fayl bilan tendensiya shakllanadi.</div>`;return}let b=trendBuckets(periods,items);let totalValues=periods.map((p,i)=>items.reduce((a,g)=>a+(g.series[i].value||0),0));let totalWeights=periods.map((p,i)=>items.reduce((a,g)=>a+(g.series[i].weight||0),0));el.innerHTML=`<h3>Tovarlar bo'yicha davriy tendensiya</h3><div class="overview-note">${periodRangeCaption(periods)}. Har bir tovar TIF TN (HS) kodi bo'yicha guruhlangan.</div><div class="kpis"><div class="kpi"><span>Jami tovar turlari</span><b>${fmtI(items.length)}</b></div><div class="kpi"><span>So'nggi davrda faol</span><b>${fmtI(b.active.length)}</b></div><div class="kpi"><span>Chiqib ketgan</span><b>${fmtI(b.stoppedAll.length)}</b></div><div class="kpi"><span>Yangi qo'shilgan</span><b>${fmtI(b.newAll.length)}</b></div></div><div class="panel"><h3>Umumiy tovar oqimi tendensiyasi (${periods[0]} - ${periods[periods.length-1]})</h3>${svgLineChart(periods,[{label:"Qiymat (ming $)",values:totalValues},{label:"Vazn (tn)",values:totalWeights}])}<div class="overview-note">Har qaysi tovar qatoriga bosilganda shu tovarning alohida qiymat/vazn tendensiyasi ochiladi.</div></div><div class="grid2">${goodsTrendTable(periods,b.stoppedAll,"IBKdan chiqib ketgan tovarlar (oldin bo'lgan, so'nggi davrda yo'q)","Barcha tovarlar faol","stopped")}${goodsTrendTable(periods,b.newAll,"Yangi qo'shilgan tovarlar (so'nggi davrda birinchi marta paydo bo'lgan)","Yangi tovar aniqlanmadi","new")}${goodsTrendTable(periods,b.stopped2,"Oxirgi 2 davrda IBKdan chiqib ketgan tovarlar","Yo'q","stopped")}${goodsTrendTable(periods,b.new2,"Oxirgi 2 davrda yangi qo'shilgan tovarlar","Yo'q","new")}</div>`}catch(e){el.innerHTML=`<h3>Tovarlar bo'yicha davriy tendensiya</h3><div class="muted">Ma'lumotni yuklab bo'lmadi: ${esc(e.message||e)}</div>`}}
function showGoodsTrend(hs){let g=(GOODS_TRENDS.goods||[]).find(x=>x.hs_code===hs);if(!g)return;let periods=GOODS_TRENDS.periods||[];let values=g.series.map(s=>s.value),weights=g.series.map(s=>s.weight);dlgTitle.textContent=`${g.goods||g.hs_code} - davriy tendensiya (TIF TN: ${g.hs_code})`;let rows=g.series.map((s,i)=>({date:periods[i],value:s.value,weight:s.weight,partiya:s.partiya,key:{}}));dlgBody.innerHTML=`<div class="overview-note">${periodRangeCaption(periods)}.</div>`+svgLineChart(periods,[{label:"Qiymat (ming $)",values},{label:"Vazn (tn)",values:weights}])+table([{k:"date",t:"Davr",w:"110px"},{k:"value",t:"Qiymat (ming $)",n:1,f:fmtN},{k:"weight",t:"Vazn (tn)",n:1,f:fmtN},{k:"partiya",t:"Partiya",n:1,f:fmtI}],rows,"fixed-table");dlg.showModal()}

let WAREHOUSE_TRENDS={},TRANSPORT_TRENDS={},TRANSPORT_CO_TRENDS={};
async function loadWarehouseTrends(){let el=$("warehouseTrendPanel");if(!el)return;try{let j=await api("/api/warehouse_trends");WAREHOUSE_TRENDS=j;let periods=j.periods||[],warehouses=j.warehouses||[];if(!periods.length){el.innerHTML=`<h3>Omborlar bo'yicha yuk oqimi tendensiyasi</h3><div class="muted">Hali tarixiy ma'lumot yetarli emas. Har bir yangi yuklangan fayl bilan tendensiya shakllanadi.</div>`;return}let totalValues=periods.map((p,i)=>warehouses.reduce((a,w)=>a+(w.series[i]?.value||0),0));let totalWeights=periods.map((p,i)=>warehouses.reduce((a,w)=>a+(w.series[i]?.weight||0),0));let growing=warehouses.filter(w=>{let s=w.series;if(s.length<2)return false;let last=s[s.length-1].value,prev=s[s.length-2].value;return last>prev&&prev>0});let shrinking=warehouses.filter(w=>{let s=w.series;if(s.length<2)return false;let last=s[s.length-1].value,prev=s[s.length-2].value;return last<prev&&last>0});let whCols=[{k:"warehouse",t:"Ombor",w:"42%"},{k:"last_value",t:"So'nggi qiymat (ming $)",n:1,f:fmtN},{k:"last_weight",t:"Vazn (tn)",n:1,f:fmtN},{k:"last_partiya",t:"Partiya",n:1,f:fmtI},{k:"trend",t:"Tendensiya",w:"90px"}];let whRows=warehouses.map(w=>{let s=w.series;let last=s[s.length-1]||{},prev=s.length>1?s[s.length-2]:{};let delta=last.value-(prev.value||0);let trend=delta>0?"📈 o'sish":delta<0?"📉 pasayish":"➡ barqaror";return {key:{},warehouse:w.warehouse,last_value:last.value||0,last_weight:last.weight||0,last_partiya:last.partiya||0,trend,_onclick:`showWarehouseTrend(${JSON.stringify(w.warehouse)})`}});el.innerHTML=`<h3>Omborlar bo'yicha yuk oqimi tendensiyasi</h3><div class="overview-note">${periodRangeCaption(periods)}. Har qaysi ombor qatoriga bosish orqali alohida tendensiya grafiği ochiladi.</div><div class="kpis"><div class="kpi"><span>Jami omborlar</span><b>${fmtI(warehouses.length)}</b></div><div class="kpi"><span>So'nggi davrda faol</span><b>${fmtI(warehouses.filter(w=>w.series[w.series.length-1]?.value>0).length)}</b></div><div class="kpi"><span>Yuk oqimi o'sgan</span><b style="color:#16a34a">${fmtI(growing.length)}</b></div><div class="kpi"><span>Yuk oqimi kamaygan</span><b style="color:#dc2626">${fmtI(shrinking.length)}</b></div></div><div class="panel"><h3>Barcha omborlar bo'yicha umumiy yuk oqimi tendensiyasi</h3>${svgLineChart(periods,[{label:"Qiymat (ming $)",values:totalValues},{label:"Vazn (tn)",values:totalWeights}])}</div><div class="panel wide"><h3>Omborlar kesimida so'nggi holat</h3><table class="fixed-table"><colgroup><col style="width:42%"><col style="width:17%"><col style="width:15%"><col style="width:12%"><col style="width:14%"></colgroup><thead><tr><th class="text">Ombor</th><th class="num">Qiymat (ming $)</th><th class="num">Vazn (tn)</th><th class="num">Partiya</th><th class="text">Tendensiya</th></tr></thead><tbody>${whRows.map(r=>`<tr onclick="showWarehouseTrend(${JSON.stringify(r.warehouse)})" style="cursor:pointer"><td class="text">${esc(r.warehouse)}</td><td class="num">${fmtN(r.last_value)}</td><td class="num">${fmtN(r.last_weight)}</td><td class="num">${fmtI(r.last_partiya)}</td><td class="text">${r.trend}</td></tr>`).join("")}</tbody></table></div>`}catch(e){let el2=$("warehouseTrendPanel");if(el2)el2.innerHTML=`<h3>Omborlar tendensiyasi</h3><div class="muted">Ma'lumotni yuklab bo'lmadi: ${esc(e.message||e)}</div>`}}
...
async function loadTransportTrends(){let el=$("transportTrendPanel");if(!el)return;try{let j=await api("/api/transport_trends");TRANSPORT_TRENDS=j;let periods=j.periods||[],transports=j.transports||[];if(!periods.length){el.innerHTML=`<h3>Transport turi bo'yicha davriy tendensiya</h3><div class="muted">Hali tarixiy ma'lumot yetarli emas. Har bir yangi yuklangan fayl bilan tendensiya shakllanadi.</div>`;return}let colors=["#1d72b8","#16a34a","#f59e0b","#dc2626","#7c3aed","#0891b2","#ea580c"];let allSeries=transports.map((t,i)=>({label:t.transport,values:t.series.map(s=>s.value),color:colors[i%colors.length]}));el.innerHTML=`<h3>Transport turi bo'yicha davriy tendensiya</h3><div class="overview-note">${periodRangeCaption(periods)}. Har qaysi transport turi bo'yicha qiymat (ming $) tendensiyasi.</div><div class="kpis">${transports.slice(0,4).map(t=>{let last=t.series[t.series.length-1]||{};return `<div class="kpi"><span>${esc(t.transport)}</span><b>${fmtN(last.value||0)}</b></div>`}).join("")}</div><div class="panel"><h3>Transport turlari bo'yicha qiymat tendensiyasi</h3>${svgLineChart(periods,allSeries)}</div><div class="grid2">${transports.map((t,i)=>{let values=t.series.map(s=>s.value),weights=t.series.map(s=>s.weight);return `<div class="panel"><h3 onclick="showTransportTrend(${JSON.stringify(t.transport)})" style="cursor:pointer;color:${colors[i%colors.length]}">${esc(t.transport)}</h3>${svgLineChart(periods,[{label:"Qiymat",values,color:colors[i%colors.length]},{label:"Vazn (tn)",values:weights}])}</div>`}).join("")}</div>`}catch(e){let el2=$("transportTrendPanel");if(el2)el2.innerHTML=`<h3>Transport turi tendensiyasi</h3><div class="muted">Ma'lumotni yuklab bo'lmadi: ${esc(e.message||e)}</div>`}}
...
async function loadTransportCompanyTrends(){let el=$("transportCompanyPanel");if(!el)return;try{let j=await api("/api/transport_company_trends");TRANSPORT_CO_TRENDS=j;let periods=j.periods||[],transports=j.transports||[];if(!periods.length){el.innerHTML=`<h3>Transport kesimida korxonalar</h3><div class="muted">Hali tarixiy ma'lumot yetarli emas.</div>`;return}let colors=["#1d72b8","#16a34a","#f59e0b","#dc2626","#7c3aed","#0891b2","#ea580c"];el.innerHTML=`<h3>Eng tez oborotli korxonalar — transport turlari kesimida</h3><div class="overview-note">${periodRangeCaption(periods)}. Har bir transport turi bo'yicha qiymat va vazn tendensiyasida TOP korxonalar.</div>${transports.map((t,ti)=>{let cols=[{k:"company",t:"Korxona",w:"45%"},{k:"stir",t:"STIR",w:"90px"},{k:"total_value",t:"Jami qiymat (ming $)",n:1,f:fmtN},{k:"last_value",t:"So'nggi davr (ming $)",n:1,f:fmtN},{k:"trend",t:"Tendensiya",w:"80px"}];let rows=t.companies.slice(0,15).map(c=>{let s=c.series,last=s[s.length-1]||{},prev=s.length>1?s[s.length-2]:{};let delta=last.value-(prev.value||0);return {key:{stir:c.stir},company:c.company||c.stir,stir:c.stir,total_value:c.total_value,last_value:last.value||0,trend:delta>0?"📈":delta<0?"📉":"➡",_onclick:`showCo_${ti}('${c.stir}')`}});let totalV=t.companies.reduce((a,c)=>a+c.total_value,0);return `<div class="panel wide"><h3 style="color:${colors[ti%colors.length]}">${esc(t.transport)} — TOP korxonalar (jami: ${fmtN(totalV)} ming $)</h3>${table(cols,rows,"fixed-table")}</div>`}).join("")}`}catch(e){let el2=$("transportCompanyPanel");if(el2)el2.innerHTML=`<h3>Transport kesimida korxonalar</h3><div class="muted">Ma'lumotni yuklab bo'lmadi: ${esc(e.message||e)}</div>`}}
...
function warehouseTurnoverRows(data){
  ...
  all.forEach(r=>{
    let name=r.warehouse||"Ombor ko'rsatilmagan";
