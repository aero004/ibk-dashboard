...
function translateRuPage(){if(LANG!=="ru")return;let pairs=[["Kirish","Вход"],["Chiqish","Выход"],["Ombor ma'lumot","Складские данные"],["Fayl yuklash","Загрузка файлов"],["Umumiy","Общий"],["Korxonalar","Предприятия"],["Muddati o'tgan","Просроченные"],["Nazoratdan yechish","Снятие с контроля"],["Tovarlar","Товары"],["Omborlar","Склады"],["Rejimlar","Режимы"],["Oziq-ovqat","Продовольствие"],["Muddatlar","Сроки"],["Boshqaruv","Управление"],["Arxiv","Архив"],["To'lovlar","Платежи"],["Davlatlar bo'yicha yo'nalishlar","Маршруты по странам"],["Deklaratsiya post kodi bo'yicha tahlil","Анализ по коду поста декларации"],["Transport turi bo'yicha ulushi","Доля по виду транспорта"],["Qiymat bo'yicha TOP 30 korxona","ТОП-30 предприятий по стоимости"],["Rahbar uchun qisqa xulosa","Краткая сводка для руководителя"]];document.querySelectorAll("button,h2,h1,b,label,.muted,.btn").forEach(el=>{let s=el.childNodes.length===1?el.textContent.trim():"";let p=pairs.find(x=>x[0]===s);if(p)el.textContent=p[1]})}
