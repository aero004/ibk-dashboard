...
function initCountryFlowMap(){
  ...
  // Inject CSS animation for flow lines
  if(!document.getElementById("cfm-css")){let s=document.createElement("style");s.id="cfm-css";s.textContent=`
    ...
    .cfm-label-div{font-size:11px;font-weight:800;color:#fff;padding:3px 7px;border-radius:9px;white-space:nowrap;box-shadow:0 2px 6px rgba(0,0,0,.45);pointer-events:none;border:1.5px solid rgba(255,255,255,.6);letter-spacing:.2px}
  `;document.head.appendChild(s)}
  COUNTRY_FLOW_MAP=L.map("cfmMap",{zoomControl:true,scrollWheelZoom:false}).setView([30,50],2);
  let osmLayer=L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{maxZoom:10,attribution:"© OpenStreetMap"});
  let satLayer=L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",{maxZoom:10,attribution:"© Esri"});
  let hybridLayer=L.layerGroup([
    L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",{maxZoom:10}),
    L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{maxZoom:10,opacity:0.4})
  ]);
  osmLayer.addTo(COUNTRY_FLOW_MAP);
  L.control.layers({"🗺 Oddiy":osmLayer,"🛰 Sputnik":satLayer,"🌐 Gibrid":hybridLayer},{},{position:"topright"}).addTo(COUNTRY_FLOW_MAP);
  let UZ=[41.3,69.3];
  let maxQ=Math.max(1,...rows.map(r=>+r.qiymat||0)),maxV=Math.max(1,...rows.map(r=>+r.vazn||0));
  let uzMarker=L.circleMarker(UZ,{radius:13,fillColor:"#1769aa",color:"#fff",weight:3,fillOpacity:1}).addTo(COUNTRY_FLOW_MAP);
  uzMarker.bindPopup("<b>🇺🇿 O'zbekiston (Toshkent)</b><br>IBK nazorat punkti");
  let uzLabel=L.divIcon({className:"",html:'<div class="cfm-label-div" style="background:#1769aa">🇺🇿 O\'zbekiston</div>',iconAnchor:[-16,8]});
  L.marker(UZ,{icon:uzLabel,zIndexOffset:1000}).addTo(COUNTRY_FLOW_MAP);
  let bounds=[UZ];
  rows.forEach((r,i)=>{
    let ll=countryLatLon(r.name,i,rows.length);
    let q=+r.qiymat||0,v=+r.vazn||0,qShare=q/maxQ;
    let radius=Math.max(7,Math.min(26,7+Math.sqrt(v/maxV)*20));
    let color=qShare>=.6?"#16a34a":qShare>=.3?"#e08800":"#2563eb";
    let latlng=[ll.lat,ll.lon];
    bounds.push(latlng);
    // Animated flow line
    let pl=L.polyline([latlng,UZ],{color:color,weight:2.5,opacity:0.75,dashArray:"10 8",className:"cfm-flow"}).addTo(COUNTRY_FLOW_MAP);
    // Plane icon along the line
    let mid=[(latlng[0]+UZ[0])/2,(latlng[1]+UZ[1])/2];
    let angle=Math.atan2(UZ[0]-latlng[0],UZ[1]-latlng[1])*180/Math.PI;
    let planeIcon=L.divIcon({className:"",html:`<div style="font-size:16px;transform:rotate(${angle}deg);opacity:.8">✈</div>`,iconSize:[20,20],iconAnchor:[10,10]});
    L.marker(mid,{icon:planeIcon,interactive:false}).addTo(COUNTRY_FLOW_MAP);
    let circle=L.circleMarker(latlng,{radius,fillColor:color,color:"#fff",weight:2,fillOpacity:0.88}).addTo(COUNTRY_FLOW_MAP);
    ...
    let icon=L.divIcon({className:"",html:`<div class="cfm-label-div" style="background:${color}">${countryFlag(r.name)} ${esc(r.name)}</div>`,iconAnchor:[-radius-4,8]});
    L.marker(latlng,{icon,zIndexOffset:500}).addTo(COUNTRY_FLOW_MAP);
  });
  try{COUNTRY_FLOW_MAP.fitBounds(bounds,{padding:[30,30],maxZoom:5})}catch(e){}
}
