...
function ratingTable(rows,valKey,valLabel,fmt){rows=(rows||[]).filter(r=>(+r[valKey]||0)>0).slice(0,20);let max=rows.reduce((m,r)=>Math.max(m,+r[valKey]||0),0)||1;return `<div class="rating-list"><div class="rating-header"><span>Korxona nomi</span><span>${esc(valLabel)}</span></div>${rows.map((r,i)=>{let pct=(+r[valKey]||0)/max*100,opacity=(Math.max(0.45,0.92-i*0.024)).toFixed(2);return `<div class="rating-row" style="opacity:${opacity}" onclick='detail(${JSON.stringify(r.key||{}).replaceAll("'","&#39;")})'><div class="rating-rank">${i+1}</div><div class="rating-name">${esc(r.korxona||r.name||'')}</div><div class="rating-bar-wrap"><div class="rating-bar" style="width:${Math.max(4,pct).toFixed(1)}%"></div><span class="rating-val">${fmt(r[valKey])}</span></div></div>`}).join('')}</div>`}
