...
function _trWalk(map){
  ...
  function walk(el){if(!el)return;for(const n of [...el.childNodes]){if(n.nodeType===3){let t=n.textContent,ch=false;for(const k of keys){if(t.includes(k)){t=t.split(k).join(map[k]);ch=true;}}if(ch)n.textContent=t;}else if(n.nodeType===1&&!['SCRIPT','STYLE','INPUT','TEXTAREA','SELECT'].includes(n.tagName))walk(n);}}
  walk($('app'));
}
