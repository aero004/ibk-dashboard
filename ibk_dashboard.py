...
function renderKpis(){let k=DATA.kpis||{};
  ...
  let foodQiymat=(DATA.food||[]).reduce((a,r)=>a+(+r.qiymat||0),0);
  let foodHtml=(DATA.food||[]).length?`<div class=kpi onclick="TAB='food';GROUP='bnrte';render()" style="cursor:pointer;border-top:3px solid #16a34a"><span>🥗 Iste'mol tovarlari</span><b style="color:#16a34a">${fmtN(foodQiymat)}</b><div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(0,0,0,.08);font-size:12px;color:var(--muted)">ming $ qiymat</div></div>`:"";
  let vaqCount=((VAQTINCHA_DATA&&VAQTINCHA_DATA.items)||[]).length;
  let vaqHtml=VAQTINCHA_DATA&&VAQTINCHA_DATA.loaded?`<div class=kpi onclick="TAB='vaqtincha';GROUP='bnrte';render()" style="cursor:pointer;border-top:3px solid #6366f1"><span>⏱ IM42/EK12</span><b style="color:#6366f1">${fmtI(vaqCount)}</b><div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(0,0,0,.08);font-size:12px;color:var(--muted)">jami yozuv</div></div>`:"";
  let wrHtml=(WR_DATA||[]).length?`<div class=kpi onclick="TAB='ombor';GROUP='bnrte';render()" style="cursor:pointer;border-top:3px solid #f59e0b"><span>🏢 Omborlar reestri</span><b style="color:#f59e0b">${fmtI(WR_DATA.length)}</b><div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(0,0,0,.08);font-size:12px;color:var(--muted)">jami ombor</div></div>`:"";
  let bkoHtml=BKO_SUMMARY&&BKO_SUMMARY.total_bko?`<div class=kpi onclick="GROUP='common';TAB='upload';render()" style="cursor:pointer;border-top:3px solid #059669"><span>📑 BKO</span><b style="color:#059669">${fmtI(BKO_SUMMARY.total_bko)}</b><div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(0,0,0,.08);font-size:12px;color:var(--muted)">jami band</div></div>`:"";
  return rows.map(x=>`<div class=kpi onclick="showKpi('${x[2]}')"><span>${x[0]}</span><b>${x[1]}</b></div>`).join("")+depHtml+aviaQiymatHtml+aviaHtml+foodHtml+vaqHtml+wrHtml+bkoHtml;}
