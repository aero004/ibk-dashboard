...
function _trWalk(map){
  function walk(el){if(!el)return;for(const n of [...el.childNodes]){if(n.nodeType===3){const t=n.textContent.trim();if(t&&map[t])n.textContent=n.textContent.replace(t,map[t]);}else if(n.nodeType===1&&!['SCRIPT','STYLE','INPUT','TEXTAREA','SELECT'].includes(n.tagName))walk(n);}}
  walk($('app'));
}
...
function translateUzcPage(){
  if(LANG!=='uzc')return;
  function walk(el){if(!el)return;for(const n of [...el.childNodes]){if(n.nodeType===3){if(/[a-zA-Z]/.test(n.textContent))n.textContent=latinToUzCyr(n.textContent);}else if(n.nodeType===1&&!['SCRIPT','STYLE','INPUT','TEXTAREA','SELECT'].includes(n.tagName))walk(n);}}  
  walk($('app'));
}
