...
function overviewPanels(){let topCompanies=by(DATA.top_value||[],"qiymat").slice(0,30), topGoods=topNWithOther(DATA.goods||[],30,"partiya","name");return `<div class="grid2">${executiveSummary()}<div class=panel><h2>70-74-80: postlar kesimida</h2>${table([{k:"post",t:"Post",w:"42%"},{k:"partiya",t:"Partiya",n:1,f:fmtI},{k:"vazn",t:"Vazn (tn)",n:1,f:fmtN},{k:"qiymat",t:"Qiymat (ming $)",n:1,f:fmtN},{k:"tolov",t:"Kutilayotgan to'lov (mln so'm)",n:1,f:fmtN}],basicTotal(DATA.all_post_summary||[],"IBK bo'yicha Jami","post"),"fixed-table")}<div class=overview-note>Post qatorlari ustiga bosilganda asos deklaratsiyalar ochiladi.</div></div><div class="panel wide"><h2>Jami muddati o'tgan: postlar va rejimlar kesimida</h2>${expiredTotalExcelTable()}</div><div class="panel wide"><h2>Qiymat bo'yicha TOP 30 korxona</h2>${bars(topCompanies,"korxona","qiymat",fmtN)}</div>${chartBlock("Muddati o'tgan partiyalar (postlar kesimida)",DATA.post_summary||[],"post","partiya",fmtI)}${chartBlock("Tovar guruhlari partiya bo'yicha ulushi",topGoods,"name","partiya",fmtI)}<div class=panel><h2>Davlatlar bo'yicha tahlil</h2>${bars(countryRows(),"name","qiymat",fmtN)}</div></div><div class="panel wide"><h2>IBKga tovarlar oqimi</h2><iframe src="/assets/tovarlar_oqimi.html?token=${TOKEN}" style="width:100%;height:640px;border:none;border-radius:12px;display:block" id="tovarlarOqimiFrame"></iframe></div>`}
