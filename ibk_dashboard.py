...
return `<tr style="border-bottom:1px solid var(--line)"><td style="padding:7px 9px;font-family:monospace;font-size:12px">${su}</td><td style="padding:7px 9px">${esc(u.full_name||'')}</td><td style="padding:7px 9px">${esc(roleTitle(u.role))}</td><td style="padding:7px 9px;text-align:center">${esc(u.post_code||'')}</td><td style="padding:7px 9px;font-size:11px;color:#5a7a9a">${(u.perms||[]).map(p=>esc(permTitle(p))).join(', ')}</td><td style="padding:4px 8px;text-align:center;white-space:nowrap;display:flex;gap:4px;justify-content:center;align-items:center"><button class="icon-btn edit-btn" onclick="editUser('${ju}')" title="Tahrirlash">${SVG_EDIT}</button><button class="icon-btn" style="background:#f0fdf4;color:#166534;border-color:#bbf7d0" onmouseover="this.style.background='#bbf7d0'" onmouseout="this.style.background='#f0fdf4'" onclick="changePassword('${ju}')" title="Parol o'zgartirish">${SVG_KEY}</button><button class="icon-btn del-btn" onclick="deleteUser('${ju}')" title="O'chirish">${SVG_DEL}</button></td></tr>`}).join('');
