...
function renderCompNewStopped(m){COMP_PERIOD_M=m;let el=$('compNewStoppedSect');if(!el||!COMPANY_TRENDS)return;let periods=COMPANY_TRENDS.periods||[],companies=COMPANY_TRENDS.companies||[];let{newItems,stoppedItems}=filterPeriodItems(periods,companies,m);el.innerHTML=`<div style="display:flex;gap:8px;align-items:center;margin:4px 0 12px;flex-wrap:wrap"><span style="font-size:12px;color:#557086;font-weight:700">Hisobot davri:</span>${periodBtns('renderCompNewStopped',m)}</div><div class="grid2">${companyTrendTable(periods,stoppedItems,'\U0001f6ab Yuk olib kelishni to\'xtatgan korxonalar','Barcha korxonalar faol','stopped')}${companyTrendTable(periods,newItems,'\U0001f195 Yangi qo\'shilgan korxonalar','Yangi korxona aniqlanmadi','new')}</div>`}
function renderGoodsNewStopped(m){GOODS_PERIOD_M=m;let el=$('goodsNewStoppedSect');if(!el||!GOODS_TRENDS)return;let periods=GOODS_TRENDS.periods||[],items=GOODS_TRENDS.goods||[];let{newItems,stoppedItems}=filterPeriodItems(periods,items,m);el.innerHTML=`<div style="display:flex;gap:8px;align-items:center;margin:4px 0 12px;flex-wrap:wrap"><span style="font-size:12px;color:#557086;font-weight:700">Hisobot davri:</span>${periodBtns('renderGoodsNewStopped',m)}</div><div class="grid2">${goodsTrendTable(periods,stoppedItems,'\U0001f6ab IBKdan chiqib ketgan tovarlar','Barcha tovarlar faol','stopped')}${goodsTrendTable(periods,newItems,'\U0001f195 Yangi qo\'shilgan tovarlar','Yangi tovar aniqlanmadi','new')}</div>`}
