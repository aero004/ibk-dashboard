...
function translateRuPage(){if(LANG!=="ru")return;let pairs=[["Kirish","????"],["Chiqish","?????"],["Ombor ma'lumot","????????? ??????"],["Fayl yuklash","???????? ??????"],["Umumiy","?????"],["Korxonalar","???????????"],["Muddati o'tgan","????????????"],["Nazoratdan yechish","?????? ? ????????"],["Tovarlar","??????"],["Omborlar","??????"],["Rejimlar","??????"],["Oziq-ovqat","??????????????"],["Muddatlar","?????"],["Boshqaruv","??????????"],["Arxiv","?????"],["To'lovlar","???????"],["Davlatlar bo'yicha yo'nalishlar","???????? ?? ???????"],["Deklaratsiya post kodi bo'yicha tahlil","?????? ?? ???? ????? ??????????"],["Transport turi bo'yicha ulushi","???? ?? ???? ??????????"],["Qiymat bo'yicha TOP 30 korxona","???-30 ??????????? ?? ?????????"],["Rahbar uchun qisqa xulosa","??????? ?????? ??? ????????????"]];document.querySelectorAll("button,h2,h1,b,label,.muted,.btn").forEach(el=>{let s=el.childNodes.length===1?el.textContent.trim():"";let p=pairs.find(x=>x[0]===s);if(p)el.textContent=p[1]})
