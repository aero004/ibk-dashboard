...
def fetch_tas_flights() -> dict:
    # FR24 limit parametrini qat'iy 1-100 oralig'ida talab qiladi (200 so'ralsa 400 Bad Request
    # qaytaradi); 100 - ruxsat etilgan maksimum, natija "bugun"ga qarab real filtrlanadi
    url = "https://api.flightradar24.com/common/v1/airport.json?code=TAS&plugin[]=schedule&limit=100"
    now = int(time.time())
...
_VALID_INNER  = {".xlsx", ".xls", ".html", ".htm"}

def _extract_if_archive(path: Path, tmp_dir: Path) -> Path:
    ...
    if ext == ".zip":
        with zipfile.ZipFile(path) as zf:
            ...
            if not names:
                raise ValueError("ZIP arxivida mos fayl topilmadi (.xlsx/.xls/.html)")
        ...
        try:
            ...
            with rarfile.RarFile(path) as rf:
                names = [n for n in rf.namelist()
                         if Path(n).suffix.lower() in _VALID_INNER]
                if not names:
                    raise ValueError("RAR arxivida mos fayl topilmadi")
                rf.extract(names[0], tmp_dir)
                path.unlink(missing_ok=True)
                return tmp_dir / names[0]
        except ImportError:
...
def _zip_to_sources(src: Path, report_date: datetime) -> None:
    try:
        ...
        with zipfile.ZipFile(SOURCES_DIR / zip_name, "w", zipfile.ZIP_DEFLATED) as zf:
...
def log_analytics(user: str, event: str, **kwargs) -> None:
    entry = {"ts": datetime.now().isoformat(timespec="seconds"), "user": user, "event": event}
    entry.update(kwargs)
...
def get_analytics_summary() -> dict:
    ...
    for ev in events:
        ...
        if u not in users:
            users[u] = {"logins": 0, "last_login": None, "total_sec": 0, "clicks": 0, "downloads": 0}
        t = ev.get("event", "")
        if t == "login":
            users[u]["logins"] += 1
            ts = ev.get("ts", "")
            if not users[u]["last_login"] or ts > users[u]["last_login"]:
                users[u]["last_login"] = ts
        elif t == "heartbeat":
            users[u]["total_sec"] = max(users[u]["total_sec"], ev.get("duration_sec", 0))
            users[u]["clicks"] += ev.get("clicks", 0)
        elif t == "session_end":
            users[u]["total_sec"] += ev.get("duration_sec", 0)
        elif t == "download":
            users[u]["downloads"] += 1
            downloads.append(ev)
        elif t == "tab_view":
    ...
    for d in downloads:
        k = d.get("filename", d.get("kind", "?"))
        download_counts[k] = download_counts.get(k, 0) + 1

    user_rows = [
        {
            "user": u,
            "logins": v["logins"],
            "last_login": (v["last_login"] or "")[:16].replace("T", " "),
            "total_min": round(v["total_sec"] / 60, 1),
            "clicks": v["clicks"],
            "downloads": v["downloads"],
        }
        for u, v in sorted(users.items(), key=lambda x: x[1]["logins"], reverse=True)
    ]
    return {
        "users": user_rows,
        "downloads": sorted(downloads, key=lambda x: x.get("ts", ""), reverse=True)[:100],
        "tab_counts": sorted(tab_counts.items(), key=lambda x: x[1], reverse=True),
        "download_counts": sorted(download_counts.items(), key=lambda x: x[1], reverse=True)[:20],
...
def run_job(job_id: str, source: Path, deposit: Path | None, report_date: datetime):
    ...
    try:
        ...
        _zip_to_sources(stored_source, report_date)
        stored_deposit = None
        if deposit:
...
HTML = r"""<!doctype html><html lang="uz"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="icon" href="/assets/sticker.webp" type="image/webp"><title>IBK Dashboard</title>
<style>
...
.sm-card{background:var(--card);border:1.5px solid var(--border);border-radius:9px;padding:6px 6px 7px;cursor:pointer;transition:.13s;width:178px;display:flex;flex-direction:column;align-items:center;text-align:center;gap:4px;position:relative;user-select:none}
.sm-card:hover{border-color:#3b82f6;box-shadow:0 2px 8px rgba(59,130,246,.22)}
.sm-card.sm-hid{opacity:.38}
.sm-card.sm-drag-over{border:2px dashed #3b82f6;background:#eff6ff}
.sm-card.dragging{opacity:.35;border-style:dashed}
.sm-thumb{width:166px;height:106px;overflow:hidden;border-radius:5px;border:1px solid var(--border);background:#f7f9fc;position:relative;flex-shrink:0}
.sm-thumb-inner{width:1140px;transform:scale(0.1456);transform-origin:top left;pointer-events:none;overflow:hidden}
.sm-thumb-empty{display:flex;align-items:center;justify-content:center;font-size:28px;width:100%;height:100%;color:var(--muted)}
.sm-card-name{font-weight:700;font-size:11px;color:var(--text);line-height:1.25;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:166px}
.sm-card-eye{position:absolute;top:3px;right:3px;width:18px;height:18px;border-radius:4px;border:none;background:rgba(0,0,0,.32);cursor:pointer;font-size:11px;display:none;padding:0;line-height:1;color:#fff;z-index:2}
...
.icon-btn{display:inline-flex;align-items:center;justify-content:center;width:30px;height:30px;padding:0;border-radius:7px;border:1px solid var(--line);background:#f0f4f8;color:#3d5a7a;cursor:pointer;transition:.15s}.icon-btn:hover{background:#dde8f3;color:var(--blue)}.icon-btn.del-btn{background:#fff0f0;color:#b42318;border-color:#ffd7d7}.icon-btn.del-btn:hover{background:#ffd7d7}.dark .icon-btn{background:#1e2d40;color:#9fb0c6;border-color:#2f3f55}.dark .icon-btn.del-btn{background:#2a1a1a;color:#f87171;border-color:#7f1d1d}
...
let EXPANDED_POST=new Set(), EXPANDED_REGIME=new Set();
let UPLOAD_GROUPS_OPEN={quick:true,main:true,extra:false,bko:false,archive:false};
try{Object.assign(UPLOAD_GROUPS_OPEN,JSON.parse(localStorage.uploadGroupsOpen||'{}'));}catch(e){}
const UC_ICONS={
  box:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 8l-9-5-9 5 9 5 9-5z"/><path d="M3 8v8l9 5 9-5V8"/><path d="M9 11.3l2 2 4-4"/></svg>',
  ...
  building:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="3" width="16" height="18" rx="1"/><path d="M9 8h.01M14 8h.01M9 12h.01M14 12h.01M9 16h.01M14 16h.01"/><path d="M10 21v-4h4v4"/></svg>',
  plane:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 13l7-2 4-7 2 1-2 6 6 1v2l-6 1 2 6-2 1-4-7-7-2z"/></svg>',
  ...
  document:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2h9l4 4v16H6z"/><path d="M15 2v4h4"/><path d="M9 12h6M9 16h6M9 8h2"/></svg>',
  ...
  truck:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M2 8h11v9H2z"/><path d="M13 11h4l4 3v3h-8z"/><circle cx="6" cy="18" r="1.6"/><circle cx="18" cy="18" r="1.6"/></svg>',
  archive:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="4" rx="1"/><path d="M4 8v11a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1V8"/><path d="M10 12h4"/></svg>',
  folder:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6a1 1 0 0 1 1-1h5l2 2h9a1 1 0 0 1 1 1v11a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6z"/></svg>',
};
function ucIcon(name,color){return `<span style="display:inline-flex;width:19px;height:19px;color:${color||'inherit'};vertical-align:-5px;margin-right:7px">${UC_ICONS[name]||''}</span>`}
const I18N={
...
let _sessionStart=Date.now(),_clickCount=0,_heartbeatSent=0;
document.addEventListener('click',()=>_clickCount++,{passive:true});
function startHeartbeat(){
  clearInterval(window._hbInterval);
  window._hbInterval=setInterval(async()=>{
    let dur=Math.round((Date.now()-_sessionStart)/1000);
    let clicks=_clickCount-_heartbeatSent;_heartbeatSent=_clickCount;
    try{await api('/api/analytics/event',{method:'POST',body:JSON.stringify({event:'heartbeat',duration_sec:dur,clicks,tab:TAB})});}catch(e){}
  },60000);
}
...
function staleBadge(){if(!DATA)return '';let d=(DATA.meta&&DATA.meta.date)||DATA.date||'';return d?dateFreshnessBadge(d):'';}
...
function kpiExtraBlocks(){
  ...
  let foodHtml=foodRowsAll.length?`<div class=kpi onclick="TAB='food';GROUP='bnrte';render()" style="cursor:pointer;border-top:3px solid #16a34a"><span>🥗 Iste'mol tovarlari</span><b style="color:#16a34a">${fmtN(foodQiymat)}</b><div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(0,0,0,.08);font-size:12px;color:var(--muted)">${fmtN(foodVazn)} tn · <b style="color:#dc2626">${foodOverCount}</b> muddati o'tgan</div>${bnrteDateTag}</div>`:"";
  let im42Html=VAQTINCHA_DATA&&VAQTINCHA_DATA.loaded?`<div class=kpi onclick="VAQTINCHA_REGIME='im42';TAB='vaqtincha';GROUP='bnrte';render()" style="cursor:pointer;border-top:3px solid #6366f1"><span>⏱ IM42</span><b style="color:#6366f1">${fmtI(VAQTINCHA_DATA.im42_count||0)}</b><div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(0,0,0,.08);font-size:12px;color:var(--muted)">${fmtN(VAQTINCHA_DATA.im42_value||0)} $ · <b style="color:#dc2626">${VAQTINCHA_DATA.expired_im42||0}</b> muddati o'tgan</div>${kpiDateTag(VAQTINCHA_DATA.report_date)}</div>`:"";
  let ek12Html=VAQTINCHA_DATA&&VAQTINCHA_DATA.loaded?`<div class=kpi onclick="VAQTINCHA_REGIME='ek12';TAB='vaqtincha';GROUP='bnrte';render()" style="cursor:pointer;border-top:3px solid #8b5cf6"><span>⏱ EK12</span><b style="color:#8b5cf6">${fmtI(VAQTINCHA_DATA.ek12_count||0)}</b><div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(0,0,0,.08);font-size:12px;color:var(--muted)">${fmtN(VAQTINCHA_DATA.ek12_value||0)} $ · <b style="color:#dc2626">${VAQTINCHA_DATA.expired_ek12||0}</b> muddati o'tgan</div>${kpiDateTag(VAQTINCHA_DATA.report_date)}</div>`:"";
...
function renderKpis(){let k=DATA.kpis||{};
  ...
  let {aviaHtml,foodHtml,im42Html,ek12Html,wrHtml,bkoHtml}=kpiExtraBlocks();
  return rows.map(x=>`<div class=kpi onclick="showKpi('${x[2]}')"><span>${x[0]}</span><b>${x[1]}</b><div style="margin-top:6px;padding-top:6px;border-top:1px solid rgba(0,0,0,.08);font-size:12px;color:var(--muted)">${x[3]}: <b style="color:var(--blue)">${x[4]}</b></div>${bnrteDateTag}</div>`).join("")+depHtml+aviaHtml+foodHtml+im42Html+ek12Html+wrHtml+bkoHtml;}
...
function showKpi(kind){if(!DATA)return;let titles={partiya:"Partiya kelib chiqishi",vazn:"Vazn kelib chiqishi",qiymat:"Qiymat kelib chiqishi",tolov:"Kutilayotgan to'lov kelib chiqishi",depozit:"Depozit kelib chiqishi",expired:"Muddati o'tgan partiya kelib chiqishi"};dlgTitle.textContent=titles[kind]||"KPI asosi";if(kind==="depozit"){dlgBody.innerHTML=table(companyCols(),companyTotal(DATA.top_deposit||[]));dlg.showModal();return}if(kind==="expired"){dlgBody.innerHTML=table([{k:"post",t:"Post",w:"22%"},{k:"jami_partiya",t:"Jami partiya",n:1,f:fmtI},{k:"jami_qiymat",t:"Jami qiymat (ming $)",n:1,f:fmtN},{k:"expired_partiya",t:"Muddati o'tgan partiya",n:1,f:fmtI},{k:"expired_qiymat",t:"Muddati o'tgan qiymat (ming $)",n:1,f:fmtN},{k:"ulush",t:"Partiyadagi ulushi (%)",n:1,f:fmtN},{k:"im70_partiya",t:"IM70 partiya",n:1,f:fmtI},{k:"im74_partiya",t:"IM74 partiya",n:1,f:fmtI},{k:"tr80_partiya",t:"TR80 partiya",n:1,f:fmtI}],expiredPostRegimeRows());dlg.showModal();return}dlgBody.innerHTML=table([{k:"rejim",t:"Rejim",w:"24%"},{k:"partiya",t:"Partiya",n:1,f:fmtI},{k:"vazn",t:"Vazn (tn)",n:1,f:fmtN},{k:"qiymat",t:"Qiymat (ming $)",n:1,f:fmtN},{k:"tolov",t:"Kutilayotgan to'lov (mln so'm)",n:1,f:fmtN}],basicTotal(DATA.regimes||[],"IBK bo'yicha Jami","rejim"));dlg.showModal()}
 function numbered(rows){return (rows||[]).map((r,i)=>Object.assign({rn:i?i:""},r))} function dateOptions(){let seen=new Set();return ARCHIVE.filter(r=>!seen.has(r.date)&&seen.add(r.date)).map(r=>`<option value="${r.date}">${r.date}</option>`).join("")} function reportOptions(){let seen=new Set();return ARCHIVE.filter(r=>!seen.has(r.date)&&seen.add(r.date)).map(r=>`<option value="${r.id}"${DATA&&r.id===DATA.id?' selected':''}>${r.date}</option>`).join("")} function xls(kind){return DATA?`<a class="btn light" href="/api/export?kind=${kind}&report=${DATA.id}&token=${TOKEN}">Excel</a>`:""}
function bars(rows,label,value,fmt,topN){rows=(rows||[]).filter(r=>(+r[value]||0)>0);let max=rows.reduce((m,r)=>Math.max(m,+r[value]||0),0)||1,sum=rows.reduce((a,r)=>a+(+r[value]||0),0)||1;let row=r=>{let pct=(+r[value]||0)/sum*100,w=(+r[value]||0)/max*100;return `<div class=barrow><div title="${esc(r[label])}" translate="no">${esc(r[label])}</div><div class=bar><span style="width:${w}%">${pct.toFixed(1)}%</span></div><b>${fmt(r[value])}</b></div>`};if(topN&&rows.length>topN+1){let sorted=rows.slice().sort((a,b)=>(+b[value]||0)-(+a[value]||0));let shown=sorted.slice(0,topN),rest=sorted.slice(topN);let restSum=rest.reduce((a,r)=>a+(+r[value]||0),0);let uid='barsRest'+Math.random().toString(36).slice(2,8);return `<div class=bars>${shown.map(row).join("")}<div class=barrow style="cursor:pointer" onclick="document.getElementById('${uid}').classList.toggle('hidden')"><div class=muted>+ ${rest.length} ta — ${(restSum/sum*100).toFixed(1)}% (bosing)</div><div class=bar></div><b></b></div><div id="${uid}" class="hidden">${rest.map(row).join("")}</div></div>`}return `<div class=bars>${rows.map(row).join("")}</div>`} function by(rows,k){return (rows||[]).slice().sort((a,b)=>(+b[k]||0)-(+a[k]||0))} function expiredBlockTable(rows){rows=rows||[];let tot={partiya:0,qiymat:0,vazn:0,tolov:0};rows.forEach(r=>{if(r.korxona){tot.partiya+=(+r.partiya||0);tot.qiymat+=(+r.qiymat||0);tot.vazn+=(+r.vazn||0);tot.tolov+=(+r.tolov||0);}});let totalRow=`<tr class="total-row" style="background:#e8f2fb;font-weight:700"><td colspan="3"><b>IBK bo'yicha Jami</b></td><td class="num">${fmtI(tot.partiya)}</td><td class="num">${fmtN(tot.qiymat)}</td><td class="num">${fmtN(tot.vazn)}</td><td class="num">${fmtN(tot.tolov)}</td><td></td></tr>`;let body=rows.map(r=>{let group=!r.korxona&&!r.stir&&r.name;if(group)return `<tr class="merged-row"><td colspan="3" title="${esc(r.name)}">${esc(r.name)}</td><td class="num">${fmtI(r.partiya)}</td><td class="num">${fmtN(r.qiymat)}</td><td class="num">${fmtN(r.vazn)}</td><td class="num">${fmtN(r.tolov)}</td><td></td></tr>`;return `<tr title="${esc((r.korxona||'')+' '+(r.reason||''))}"><td class="num">${esc(r.name)}</td><td class="text col-korxona">${esc(r.korxona)}</td><td class="num">${esc(r.stir)}</td><td class="num">${fmtI(r.partiya)}</td><td class="num">${fmtN(r.qiymat)}</td><td class="num">${fmtN(r.vazn)}</td><td class="num">${fmtN(r.tolov)}</td><td class="text col-reason" translate="no">${esc(r.reason)}</td></tr>`}).join("");return `<table class="sample-like expired-detail-sample"><colgroup><col style="width:44px"><col style="width:330px"><col style="width:116px"><col style="width:74px"><col style="width:96px"><col style="width:96px"><col style="width:116px"><col style="width:320px"></colgroup><thead><tr><th rowspan=2>T/r</th><th colspan=2>Korxona ma'lumotlari</th><th rowspan=2>Partiya</th><th rowspan=2>Qiymati<br>(ming $)</th><th rowspan=2>Vazni<br>(tn)</th><th rowspan=2>Kutilayotgan<br>(mln so'm)</th><th rowspan=2>Saqlanish sababi</th></tr><tr><th>Korxona nomi</th><th>STIR</th></tr></thead><tbody>${totalRow}${body}</tbody></table>`}
function uploadPanel(){return `<div class=stack><div class=panel><h2>Fayl yuklash</h2><div class=muted>Bu modul umumiy: BNRTE va To'lovlar uchun fayllar alohida yuklanadi.</div></div><div class=panel><h2>BNRTE jamlanma</h2><form class="upload" id="upload"><div><label>Asos fayl</label><input name="source" type="file" accept=".xls,.xlsx,.html,.htm" required></div><div><label>Depozit fayl</label><input name="deposit" type="file" accept=".xlsx"></div><div></div><button>BNRTE yuklash</button></form><div class=muted>Hisobot sanasi asos fayl nomidan avtomatik aniqlanadi.</div></div><div class=panel><h2>To'lovlar jadvallari</h2><form class="upload" id="tolovUpload"><div><label>To'lov baza fayli</label><input name="source" type="file" accept=".xlsx,.xls,.zip,.rar" required></div><div class=muted>04.06+07.06.2026 kabi asos fayl yuklanadi.</div><div></div><button>To'lov jadvallarini shakllantirish</button></form><div id="tolovUploadResult" class=muted>Natijada 13 ta Excel fayl shakllanadi va To'lovlar modulidagi yuklash tugmalariga ulanadi.</div></div><div class=panel><h2>Omborlar reestri</h2><div class="wr-upload-form"><label style="font-size:13px;font-weight:600">Yangi reestri faylini yuklash:</label><input type="file" id="wrFile" accept=".xlsx,.xls,.zip,.rar" style="font-size:12px"><button onclick="uploadWrRegistry()">Yuklash</button><span class="muted" id="wrUploadStatus"></span></div><div class=muted style="font-size:12px">omborlarReestri*.xlsx formatidagi fayl yuklanadi. Yuklangandan so'ng Omborlar → Reestri bo'limida aks etadi.</div></div><div class=panel><h2>✈ AVIA AWB</h2><div class="wr-upload-form"><label style="font-size:13px;font-weight:600">AWB Excel yuklash:</label><input type="file" id="aviaFile" accept=".xlsx,.xls,.zip,.rar" style="font-size:12px" onchange="uploadAviaAwbDirect(this.files[0])"><span class="muted" id="aviaUploadStatus"></span></div><div class=muted style="font-size:12px">"Yuklarni qabul qilish*.xlsx" formatidagi fayl yuklanadi. Yuklangandan so'ng BNRTE → AVIA AWB bo'limida aks etadi.</div></div></div>`} function bindUpload(){let f=$("upload");if(f)f.onsubmit=async e=>{e.preventDefault();$("status").textContent="BNRTE fayllari yuklanyapti...";let j=await api("/api/reports",{method:"POST",body:new FormData(f)});poll(j.job_id)};let tf=$("tolovUpload");if(tf)tf.onsubmit=async e=>{e.preventDefault();$("status").textContent="To'lovlar shakllantirilyapti...";let j=await api("/api/tolov",{method:"POST",body:new FormData(tf)});PAYMENTS=j.payments||[];$("tolovUploadResult").innerHTML=`Tayyor: ${fmtI(PAYMENTS.reduce((a,r)=>a+(+r.rows||0),0))} qator, ${fmtN(PAYMENTS.reduce((a,r)=>a+(+r.sum||0),0))} so'm. <button class="light" onclick="GROUP='payments';TAB='pay_lists';render()">To'lovlar jadvaliga o'tish</button>`;$("status").textContent="To'lovlar tayyor"}}
...
if(TAB==="yaroqlilik"){
  ...
  if(!yd.loaded){
    ...
    let foodHtml=foodRows.length&&!slAll.length?`<div class="panel wide" style="margin-top:12px"><h2>Oziq-ovqat: 3 oy+ saqlanayotganlar (asos fayldan) ${staleBadge()}</h2><div class=muted style="margin-bottom:8px">Iste'mol muddati xavf ostida bo'lishi mumkin bo'lgan tovarlar.</div>${table([{k:"name",t:"Tovar turi",w:"40%"},{k:"over_vazn",t:"3 oy+ vazn (tn)",n:1,f:fmtN},{k:"over_qiymat",t:"3 oy+ qiymat (ming $)",n:1,f:fmtN},{k:"vazn",t:"Jami vazn (tn)",n:1,f:fmtN},{k:"qiymat",t:"Jami qiymat (ming $)",n:1,f:fmtN}],foodRows)}</div>`:'';
    v.innerHTML=`<div class=stack><div class=panel><h2>Iste'mol muddati tahlili</h2><div class=muted>Asos fayldan avtomatik aniqlangan iste'mol muddatlari quyida ko'rsatildi.</div></div>${slHtml}${foodHtml}</div>`;
    return;
  }
...
function adminUsersHtml(){return `<div class="admin-layout"><div class="admin-card"><h2>Yangi xodim</h2><form id=userForm class=admin-form><input name=user placeholder="Login" required><input name=password placeholder="Dastlabki parol" required><input name=full_name placeholder="F.I.Sh."><input name=position placeholder="Lavozim"><input name=phone placeholder="Telefon"><input name=post_code placeholder="Post kodi, masalan 00102"><select name=role><option value=foydalanuvchi>Foydalanuvchi</option><option value=inspektor>Inspektor</option><option value=rahbar>Rahbar</option><option value=admin>Admin</option></select><select name=lang><option value=uz>O'zbek lotin</option><option value=uzc>O'zbek kirill</option><option value=ru>Rus tili</option></select><div class=perm-grid><label><input type=checkbox name=perm_view checked> Ko'rish</label><label><input type=checkbox name=perm_upload> Yuklash</label><label><input type=checkbox name=perm_export> Eksport</label><label><input type=checkbox name=perm_release> Yechish</label></div><button>Saqlash</button></form></div><div class="admin-card"><h2>Hodimlar ro'yxati</h2><div id=users></div></div></div>`}
async function loadStatistika(){let box=$('adminView');if(!box)return;try{let j=await api('/api/analytics');let uRows=(j.users||[]).map(u=>`<tr><td>${esc(u.user)}</td><td class=num>${u.logins}</td><td>${esc(u.last_login||'—')}</td><td class=num>${u.total_min}</td><td class=num>${u.clicks}</td><td class=num style="color:${u.downloads>0?'#1d72b8':'inherit'}">${u.downloads}</td></tr>`).join('');let dRows=(j.download_counts||[]).map(([name,cnt])=>`<tr><td class=text style="max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${esc(name)}">${esc(name)}</td><td class=num>${cnt}</td></tr>`).join('');let tabRows=(j.tab_counts||[]).map(([tab,cnt])=>`<tr><td>${esc(tab)}</td><td class=num>${cnt}</td></tr>`).join('');let dlRows=(j.downloads||[]).slice(0,50).map(d=>`<tr><td>${esc(d.user||'?')}</td><td class=text style="max-width:220px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(d.filename||d.kind||'?')}</td><td>${esc((d.ts||'').slice(0,16).replace('T',' '))}</td></tr>`).join('');box.innerHTML=`<div class=stack><div class=panel><h2>Foydalanuvchilar faolligi</h2><p class=muted style="font-size:12px">Jami ${j.total_events||0} ta hodisa qayd etilgan.</p><div style="overflow-x:auto"><table class="fixed-table"><thead><tr><th>Login</th><th>Kirish soni</th><th>Oxirgi kirish</th><th>Saytda vaqt (min)</th><th>Kliklar</th><th>Yuklamalar</th></tr></thead><tbody>${uRows||'<tr><td colspan=6 class=muted>Hali ma\'lumot yo\'q</td></tr>'}</tbody></table></div></div><div class="grid2"><div class=panel><h2>Ko'p yuklangan fayllar</h2><div style="overflow-x:auto"><table class="fixed-table"><thead><tr><th>Fayl nomi</th><th>Marta</th></tr></thead><tbody>${dRows||'<tr><td colspan=2 class=muted>—</td></tr>'}</tbody></table></div></div><div class=panel><h2>Tab ochiblar soni</h2><table class="fixed-table"><thead><tr><th>Tab</th><th>Marta</th></tr></thead><tbody>${tabRows||'<tr><td colspan=2 class=muted>—</td></tr>'}</tbody></table></div></div><div class=panel><h2>So'nggi yuklamalar tarixi</h2><div style="overflow-x:auto"><table class="fixed-table"><thead><tr><th>Foydalanuvchi</th><th>Fayl</th><th>Sana/vaqt</th></tr></thead><tbody>${dlRows||'<tr><td colspan=3 class=muted>Hali yuklama yo\'q</td></tr>'}</tbody></table></div></div></div>`}catch(e){if(box)box.innerHTML=`<div class=panel><p class=muted>Xatolik: ${esc(String(e))}</p></div>`}}function bindUserForm(){let f=$("userForm");if(!f)return;f.onsubmit=async e=>{e.preventDefault();let perms=[];["view","upload","export","release"].forEach(p=>{if(f[`perm_${p}`]?.checked)perms.push(p)});await api("/api/users",{method:"POST",body:JSON.stringify({user:f.user.value,pass:f.password.value,full_name:f.full_name.value,position:f.position.value,phone:f.phone.value,post_code:f.post_code.value,role:f.role.value,lang:f.lang.value,perms})});loadUsers();f.reset();f.perm_view.checked=true}} async function loadUsers(){let box=$("users");if(!box)return;try{let j=await api("/api/users");let rows=(j.users||[]).map(u=>Object.assign({},u,{role_label:`<span class="role-pill">${roleTitle(u.role)}</span>`,perms_text:(u.perms||[]).map(permTitle).join(", ")}));box.innerHTML=table([{k:"user",t:"Login",w:"90px"},{k:"full_name",t:"F.I.Sh.",w:"210px"},{k:"role_label",t:"Rol",w:"120px"},{k:"post_code",t:"Post",w:"75px"},{k:"perms_text",t:"Vakolatlar",w:"260px"}],rows,"fixed-table").replaceAll("&lt;span class=&quot;role-pill&quot;&gt;","<span class=\"role-pill\">").replaceAll("&lt;/span&gt;","</span>")}catch(e){box.innerHTML="Admin vakolati kerak"}}
function svgLineChart(periods,series,mode='absolute'){let w=900,h=230,padL=56,padR=16,padT=18,padB=34;let plotSeries=series;let origVals=series.map(s=>s.values);if(mode==='indexed'){plotSeries=series.map(s=>{let base=s.values.find(v=>+v);return Object.assign({},s,{values:base?s.values.map(v=>(+v||0)/base*100):s.values.map(()=>0)})})}let allVals=plotSeries.flatMap(s=>s.values);let maxV=Math.max(1,...allVals);let n=Math.max(periods.length,1);let x=i=>padL+(n>1?i/(n-1)*(w-padL-padR):(w-padL-padR)/2);let y=val=>h-padB-(val/maxV)*(h-padT-padB);let grid=[0,.25,.5,.75,1].map(f=>{let val=maxV*f,yy=y(val);return `<line x1="${padL}" y1="${yy.toFixed(1)}" x2="${w-padR}" y2="${yy.toFixed(1)}" class="trend-grid"/><text x="${padL-8}" y="${(yy+4).toFixed(1)}" text-anchor="end" class="trend-axis">${mode==='indexed'?Math.round(val):fmtN(val)}</text>`}).join("");let lines=plotSeries.map((s,si)=>{let pts=s.values.map((v,i)=>`${x(i).toFixed(1)},${y(v).toFixed(1)}`).join(" ");let dots=s.values.map((v,i)=>`<circle cx="${x(i).toFixed(1)}" cy="${y(v).toFixed(1)}" r="3.6" class="trend-dot c${si}"><title>${esc(periods[i]||"")}: ${mode==='indexed'?v.toFixed(1)+' (asl: '+fmtN(origVals[si][i])+')':fmtN(v)}</title></circle>`).join("");return `<polyline points="${pts}" class="trend-line c${si}"/>${dots}`}).join("");let xlabels=periods.map((p,i)=>{if(periods.length>10&&i%Math.ceil(periods.length/10)!==0&&i!==periods.length-1)return "";return `<text x="${x(i).toFixed(1)}" y="${h-8}" text-anchor="middle" class="trend-axis">${esc(p)}</text>`}).join("");let legend=plotSeries.map((s,si)=>`<span class="trend-legend c${si}">\u25CF ${esc(s.label)}</span>`).join(" ");return `<div class="trend-chart-wrap"><svg viewBox="0 0 ${w} ${h}" class="trend-chart" preserveAspectRatio="xMidYMid meet">${grid}${lines}${xlabels}</svg><div class="trend-legend-row">${legend}</div></div>`}
...
function filterPeriodItems(periods,items,months){if(!periods.length)return{newItems:[],stoppedItems:[]};let lastDate=new Date(periods[periods.length-1]);let cutoff=new Date(lastDate);cutoff.setMonth(cutoff.getMonth()-months);let cutoffStr=cutoff.toISOString().slice(0,10);let bi=[],wi=[];periods.forEach((p,i)=>{if(p<cutoffStr)bi.push(i);else wi.push(i)});if(!wi.length)return{newItems:[],stoppedItems:[]};if(!bi.length)return{newItems:[],stoppedItems:[]};let newItems=items.filter(item=>{let hb=bi.some(i=>seriesNZ(item.series[i]));let hw=wi.some(i=>seriesNZ(item.series[i]));return!hb&&hw});let stoppedItems=items.filter(item=>{let hb=bi.some(i=>seriesNZ(item.series[i]));let hw=wi.some(i=>seriesNZ(item.series[i]));return hb&&!hw});return{newItems,stoppedItems}}
...
let WAREHOUSE_PERIOD_M=6,TRANSPORT_PERIOD_M=3,TRANSPORT_CHART_MODE='absolute';
function setTransportChartMode(m){TRANSPORT_CHART_MODE=m;renderTransportChart(TRANSPORT_PERIOD_M);}
...
function renderTransportChart(m){TRANSPORT_PERIOD_M=m;let el=$('trChartSect');if(!el||!TRANSPORT_TRENDS?.periods?.length)return;let periods=TRANSPORT_TRENDS.periods,transports=TRANSPORT_TRENDS.transports;let fp=filterPeriodArr(periods,m);let wi=fp.map(p=>periods.indexOf(p));let colors=["#1d72b8","#16a34a","#f59e0b","#dc2626","#7c3aed","#0891b2","#ea580c"];let allSeries=transports.map((t,i)=>({label:t.transport,values:wi.map(idx=>t.series[idx]?.value||0),color:colors[i%colors.length]}));let modeBtns=`<div style="display:flex;gap:4px"><button class="${TRANSPORT_CHART_MODE==='absolute'?'btn':'btn light'}" style="padding:4px 12px;font-size:12px" onclick="setTransportChartMode('absolute')">Ming $</button><button class="${TRANSPORT_CHART_MODE==='indexed'?'btn':'btn light'}" style="padding:4px 12px;font-size:12px" onclick="setTransportChartMode('indexed')">Indeks (=100)</button></div>`;let title=TRANSPORT_CHART_MODE==='indexed'?"Transport turlari bo'yicha indekslashtirilgan tendensiya (birinchi davr = 100)":"Transport turlari bo'yicha qiymat tendensiyasi";el.innerHTML=`<div style="display:flex;gap:8px;margin:4px 0 8px;flex-wrap:wrap;align-items:center;justify-content:space-between"><div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center"><span style="font-size:12px;color:#557086;font-weight:700">Hisobot davri:</span>${periodBtns('renderTransportChart',m)}</div>${modeBtns}</div><div class="panel"><h3>${title} (${fp[0]||''} — ${fp[fp.length-1]||''})</h3>${svgLineChart(fp,allSeries,TRANSPORT_CHART_MODE)}</div>`}
...
function overviewPanels(){let topCompanies=by(DATA.top_value||[],"qiymat").slice(0,30);return `<div class="grid2">${executiveSummary()}<div class=panel><h2>70-74-80: postlar kesimida</h2>${table([{k:"post",t:"Post",w:"42%"},{k:"partiya",t:"Partiya",n:1,f:fmtI},{k:"vazn",t:"Vazn (tn)",n:1,f:fmtN},{k:"qiymat",t:"Qiymat (ming $)",n:1,f:fmtN},{k:"tolov",t:"Kutilayotgan to'lov (mln so'm)",n:1,f:fmtN}],basicTotal(DATA.all_post_summary||[],"IBK bo'yicha Jami","post"),"fixed-table")}<div class=overview-note>Post qatorlari ustiga bosilganda asos deklaratsiyalar ochiladi.</div></div><div class="panel wide"><h2>Jami muddati o'tgan: postlar va rejimlar kesimida</h2>${expiredTotalExcelTable()}</div><div class="panel wide"><h2>Qiymat bo'yicha TOP 30 korxona</h2>${bars(topCompanies,"korxona","qiymat",fmtN,5)}</div><div id="_cfmSlot"></div></div>`}
...
function startIdleTimer(){let t;function reset(){clearTimeout(t);t=setTimeout(()=>{if(_uploadActive){reset();return;}logout();alert("20 daqiqa foydalanilmagani uchun profil avtomatik yopildi.")},1200000)}["click","keydown","mousemove","scroll","touchstart"].forEach(e=>document.addEventListener(e,reset,{passive:true}));reset()}startIdleTimer();
...
function sourcePostInfographics(){let allPosts=by(namedSourcePosts(),"qiymat").slice(0,12),posts=allPosts.slice(0,5),restPosts=allPosts.slice(5);let trans=DATA.transport||[],tp=trans.reduce((a,r)=>a+(+r.qiymat||0),0)||1,pp=allPosts.reduce((a,r)=>a+(+r.qiymat||0),0)||1;let rings=trans.map((r,i)=>{let pct=Math.round((+r.qiymat||0)/tp*100),dash=Math.max(0,Math.min(100,pct));return `<div class=transport-ring style="--p:${dash};--delay:${i*.12}s" onclick='detail(${JSON.stringify(r.key||{transport:r.name})})'><b>${esc(r.name||"-")}</b><span>${pct}%</span><small>${fmtI(r.partiya||0)} partiya · ${fmtN(r.qiymat||0)} ming $</small></div>`}).join("");let flowRow=(r,i)=>{let pct=(+r.qiymat||0)/pp*100;return `<div class=flow-row onclick='detail(${JSON.stringify(r.key||{})})' title="${esc(r.post_nomi||r.post_kodi||"-")}"><div class=flow-name><b>${esc(r.post_kodi||"-")}</b><span>${esc(r.post_nomi||"-")}</span></div><div class=flow-track><i style="width:${pct.toFixed(1)}%;min-width:6px;animation-delay:${i*.05}s"></i><em>${pct.toFixed(1)}%</em></div><div class=flow-num>${fmtN(r.qiymat||0)}<span class=flow-unit>ming $</span></div></div>`};let barsHtml=posts.map(flowRow).join("");if(restPosts.length){let restSum=restPosts.reduce((a,r)=>a+(+r.qiymat||0),0);barsHtml+=`<div class=flow-row style="cursor:pointer;opacity:.75" onclick="document.getElementById('srcPostRest').classList.toggle('hidden')"><div class=flow-name><b>+ ${restPosts.length} ta post</b></div><div class=flow-track><i style="width:${(restSum/pp*100).toFixed(1)}%;min-width:6px"></i><em>${(restSum/pp*100).toFixed(1)}%</em></div><div class=flow-num>${fmtN(restSum)}<span class=flow-unit>ming $</span></div></div><div id="srcPostRest" class=hidden>${restPosts.map(flowRow).join("")}</div>`}return `<div class="panel wide"><h2>Nazoratga qo'yilgan postlar va transport turlari</h2><div class="overview-note" style="margin-bottom:6px">Barcha qiymatlar ming AQSh dollarida (ming $) berilgan. Post ulushi umumiy qiymat asosida hisoblanadi.</div><div class=transport-viz><div class=ring-grid>${rings}</div><div class=flow-list>${barsHtml}</div></div></div>`}
...
const overviewPanelsWithMap=overviewPanels;overviewPanels=function(){let html=overviewPanelsWithMap();let countries=countryRows();let countriesLatin=countries.map(r=>Object.assign({},r,{name:countryLatinName(r.name)}));let countryBlock=`<div class="panel wide"><h2>Davlatlar bo'yicha yo'nalishlar</h2>${countryFlowMap(countries)}<div class="chart-under-globe">${bars(countriesLatin,"name","qiymat",fmtN,5)}</div></div>${transportPanel()}${flightsPanelShell()}`;return html.replace('<div id="_cfmSlot"></div>',countryBlock)}
...
function _trWalk(map){
  const keys=Object.keys(map).sort((a,b)=>b.length-a.length);
  function escR(s){return s.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');}
  function walk(el){if(!el)return;for(const n of [...el.childNodes]){if(n.nodeType===3){let t=n.textContent,ch=false;for(const k of keys){if(!t.includes(k))continue;const re=new RegExp(escR(k)+'(?![a-zA-Z0-9])','g');const nt=t.replace(re,map[k]);if(nt!==t){t=nt;ch=true;}}if(ch)n.textContent=t;}else if(n.nodeType===1&&!['SCRIPT','STYLE','INPUT','TEXTAREA','SELECT'].includes(n.tagName)&&n.getAttribute('translate')!=='no')walk(n);}}
  walk($('app'));
}
function latinToUzCyr(s){
  return s
    .replace(/G’/g,'Ғ').replace(/g’/g,'ғ').replace(/O’/g,'Ў').replace(/o’/g,'ў')
    .replace(/G'/g,'Ғ').replace(/g'/g,'ғ').replace(/O'/g,'Ў').replace(/o'/g,'ў')
    .replace(/Sh/g,'Ш').replace(/SH/g,'Ш').replace(/sh/g,'ш')
    .replace(/Ch/g,'Ч').replace(/CH/g,'Ч').replace(/ch/g,'ч')
    .replace(/Ng/g,'Нг').replace(/NG/g,'НГ').replace(/ng/g,'нг')
    ...
    .replace(/C/g,'С').replace(/c/g,'с').replace(/D/g,'Д').replace(/d/g,'д')
    .replace(/E/g,'Е').replace(/e/g,'е').replace(/F/g,'Ф').replace(/f/g,'ф')
    .replace(/G/g,'Г').replace(/g/g,'г').replace(/H/g,'Ҳ').replace(/h/g,'ҳ')
...
function translateRuPage(){
  if(LANG!=='ru')return;
  const M={"Kirish": "Вход", "Chiqish": "Выход", "Saqlash": "Сохранить", "Tozalash": "Очистить", "Yuklash": "Загрузить", "Barchasi": "Все", "Yangilash": "Обновить", "Bekor qilish": "Отмена", "Filtr": "Фильтр", "Tayyor": "Готово", "Xatolik": "Ошибка", "Jami": "Итого", "Umumiy": "Общий", "Korxonalar": "Предприятия", "Omborlar": "Склады", "Rejimlar": "Режимы", "Muddati o'tgan": "Просроченные", "Nazoratdan yechish": "Снятие с контроля", "Tovarlar": "Товары", "Oziq-ovqat": "Продовольствие", "Muddatlar": "Сроки", "Yaroqlilik": "Срок годности", "Arxiv": "Архив", "To'lovlar": "Платежи", "Fayl yuklash": "Загрузка файлов", "Profil": "Профиль", "Sozlamalar": "Настройки", "Boshqaruv": "Управление", "Umumiy ko'rsatkichlar": "Общие показатели", "Postlar kesimida": "В разрезе постов", "Jami muddati o'tgan (jadval)": "Итого просрочено (таблица)", "TOP 30 korxona diagrammasi": "Диаграмма ТОП-30 предприятий", "Davlatlar tovarlar oqimi (xarita)": "Поток товаров по странам (карта)", "Jami va 70-74-80 jadvali": "Итог и таблица 70-74-80", "Rejimlar kesimida jadvali": "Таблица в разрезе режимов", "Qiymat ulushi diagrammasi": "Диаграмма доли стоимости", "To'lov ulushi diagrammasi": "Диаграмма доли платежей", "TOP reyting bloki": "Блок рейтинга ТОП", "TOP 20 (qiymat) jadvali": "Таблица ТОП-20 (по стоимости)", "TOP 20 (depozit) jadvali": "Таблица ТОП-20 (по депозиту)", "Davriy tendensiya grafigi": "График периодических тенденций", "Transport: korxonalar oqimi": "Транспорт: поток предприятий", "Transport turi tendensiyasi": "Тенденция видов транспорта", "Omborlar reytingi": "Рейтинг складов", "Omborlar reestri": "Реестр складов", "Omborlar kesimida jadvali": "Таблица в разрезе складов", "Omborlar oboroti": "Оборот складов", "Yuk oqimi tendensiyasi": "Тенденция грузопотока", "Muddatlar kesimida jadvali": "Таблица в разрезе сроков", "Saqlanish sabablari jadvali": "Таблица причин хранения", "Muddat va sabab diagrammalari": "Диаграммы сроков и причин", "Postlar va rejimlar jadvali": "Таблица постов и режимов", "Postlar kesimida jadvali": "Таблица в разрезе постов", "Jamlanma jadvali": "Сводная таблица", "Korxonalar jadvali": "Таблица предприятий", "Tovar reytingi bloki": "Блок рейтинга товаров", "Tovarlar guruhlari jadvali": "Таблица групп товаров", "Oziq-ovqatlar jadvali": "Таблица продовольственных товаров", "Arxiv — BNRTE hisobotlari": "Архив — отчёты BNRTE", "Arxiv — Boshqa fayllar": "Архив — Другие файлы", "Ma'lumotlar boshqaruvi": "Управление данными", "Barcha fayllarni birdan yuklash": "Загрузить все файлы сразу", "Asos fayl va Depozit": "Основной файл и Депозит", "To'lovlar jadvallari": "Таблицы платежей", "KPI ko'rsatkichlar": "KPI показатели", "Deklaratsiyalar jadvali": "Таблица деклараций", "Muddati o'tganlar ro'yxati": "Список просроченных", "Iste'mol muddati tahlili": "Анализ сроков годности", "Toshkent xalqaro aeroporti — jonli parvozlar": "Ташкентский аэропорт — рейсы", "Davlatlar bo'yicha yo'nalishlar": "Маршруты по странам", "Deklaratsiya post kodi bo'yicha tahlil": "Анализ по коду поста", "Transport turi bo'yicha ulushi": "Доля по видам транспорта", "Qiymat bo'yicha TOP 30 korxona": "ТОП-30 предприятий по стоимости", "Rahbar uchun qisqa xulosa": "Краткая сводка для руководителя", "Korxona": "Предприятие", "Partiya": "Партия", "Vazn (tn)": "Вес (тн)", "Qiymat (ming $)": "Стоимость (тыс. $)", "To'lov (mln so'm)": "Платёж (млн. сум)", "Depozit (mln so'm)": "Депозит (млн. сум)", "Sana": "Дата", "Tovar": "Товар", "Rejim": "Режим", "Post": "Пост", "Muddat": "Срок", "Holat": "Статус", "Deklaratsiya": "Декларация", "Ombor": "Склад", "Mamlakat": "Страна", "Transport": "Транспорт", "Ulush %": "Доля %", "Qoldiq": "Остаток", "Korxona soni": "Кол-во предприятий", "Mamlakat nomi": "Наименование страны", "Tovar nomi": "Наименование товара", "Boshlang'ich post kodi": "Код исходного поста", "Boshlang'ich post nomi": "Наименование исходного поста", "Nazorat posti": "Пост контроля", "Umumiy qoldiq": "Общий остаток", "Yangi kelgan": "Поступило", "Nazoratdan yechilgan": "Снято с контроля", "Kun hisobi": "Дней хранения", "Jami korxona": "Всего предприятий", "IBK bo'yicha Jami": "Итого по СТК", "Aniqlangan fayllar:": "Определённые файлы:", "Yuklanmoqda...": "Загрузка...", "Ma'lumot yo'q": "Нет данных", "Saqlandi": "Сохранено", "Qayta hisoblash": "Пересчитать", "Hodim qo'shish yoki tahrirlash": "Добавление или редактирование сотрудника", "Hodimlar ro'yxati": "Список сотрудников", "Server boshqaruvi": "Управление сервером", "Serverni restart qilish": "Перезапустить сервер", "Sayt xaritasi": "Карта сайта", "Ombor ma'lumot": "Складские данные", "BNRTE": "БНРТЭ", "IBK": "СТК", "Umumiy nazorat": "Общий контроль", "Eng yirik korxona": "Крупнейшее предприятие", "O'z ombor": "Собственный склад", "Depozit yetakchisi": "Лидер по депозиту", "ming $ qiymat": "тыс. $ стоимость", "mln so'm": "млн. сум", "Partiya kelib chiqishi": "Источник партий", "Vazn kelib chiqishi": "Источник веса", "Qiymat kelib chiqishi": "Источник стоимости", "Kutilayotgan to'lov kelib chiqishi": "Источник ожидаемых платежей", "Depozit kelib chiqishi": "Источник депозита", "Muddati o'tgan partiya kelib chiqishi": "Источник просроченных партий", "Korxona nomi": "Наименование предприятия", "Tovar guruhi": "Группа товаров", "Tendensiya davri": "Период тенденции", "Yangi korxonalar": "Новые предприятия", "To'xtatgan korxonalar": "Прекратившие предприятия", "Faol korxonalar": "Активные предприятия", "Yangi tovarlar": "Новые товары", "To'xtatgan tovarlar": "Прекратившие товары", "Yangi omborlar": "Новые склады", "To'xtatgan omborlar": "Прекратившие склады", "Hisob sanasi": "Дата отчёта", "Hisobot sanasi": "Дата отчёта", "Faol": "Активные", "Qisman yechilgan": "Частично снято", "Boshlang'ich sana": "Начальная дата", "Oxirgi sana": "Конечная дата", "Foydalanuvchi": "Пользователь", "Lavozim": "Должность", "Telefon": "Телефон", "Rol": "Роль", "Vakolatlar": "Полномочия", "Amal": "Действие", "navbatda": "в очереди", "qayta ishlanmoqda": "обрабатывается", "Hisobot": "Отчёт", "Avtomobil": "Автомобиль", "Jo'natuvchi post": "Пост отправки", "3 oy": "3 мес.", "6 oy": "6 мес.", "12 oy": "12 мес.", "24 oy": "24 мес.", "Umumiy ma'lumot": "Общая информация", "Vaqtincha": "Временное", "Ma'lumotlarni yangilash": "Обновить данные", "Sanani tanlang": "Выберите дату", "Ko'rish": "Просмотр", "Eksport": "Экспорт", "Yechish": "Снятие", "Rejimlar bo'yicha": "По режимам", "Postlar bo'yicha": "По постам", "Sana bo'yicha": "По дате", "Hafta": "Неделя", "Oy": "Месяц", "Yil": "Год",
   "Interaktiv hisobot": "Интерактивный отчёт",
   "Hech narsa topilmadi": "Ничего не найдено",
   "Hosil bo'lgan to'lov jadvallari": "Сформированные таблицы платежей",
   "To'lov turlari bo'yicha tahlil": "Анализ по видам платежей",
   "Eng katta summalar": "Наибольшие суммы",
   "Qatorlar soni": "Количество строк",
   "Jami qator": "Всего строк",
   "Jami summa": "Общая сумма",
   "To'lov turlari": "Виды платежей",
   "Eng katta tur": "Крупнейший вид",
   "Hammasini ZIP": "Скачать всё ZIP",
   "Summa ulushi": "Доля суммы",
   "kun o'tgan!": "дней истекло!",
   "kun qoldi": "дней осталось",
   "ta muddati o'tgan,": "просрочено,",
   "ta yaqinlashyapti": "приближается",
   "ta ombor muddati yaqinlashyapti": "складов срок приближается",
   "Ko'rinish tartibi — Drag & Drop": "Порядок отображения — Drag & Drop",
   "Sudrab joylashtiring": "Перетащите",
   "Ko'z tugmasi bilan yashiring/ko'rsating": "Нажмите глазок для скрытия/показа",
   "Asliyga qaytarish": "Восстановить",
   "Muddati o'tgan tovar": "Товар с истёкшим сроком",
   "180 kun ichida tugaydi": "Истекает в течение 180 дней",
   "1 oy ichida tugaydi": "Истекает в течение 1 месяца",
   "O'tgan tovar qiymati (ming $)": "Стоимость истёкших (тыс. $)",
   "Umumiy vazn (tonna)": "Общий вес (тонна)",
   "Hisob-kitob sanasi": "Дата расчёта",
   "ta tovar pozitsiyasi tahlil qilindi": "позиций товаров проанализировано",
   "Muddati o'tgan tovarlar — korxonalar kesimida": "Истёкшие товары — в разрезе предприятий",
   "Yuk qabul qiluvchi": "Получатель груза",
   "TIF TN kodi": "Код ТН ВЭД",
   "Yaroqlilik muddati": "Срок годности",
   "Muddat holati": "Статус срока",
   "Pozitsiyalar": "Позиции",
   "BNRTE jamlanma": "БНРТЭ — сводная",
   "Asos fayl": "Основной файл",
   "Depozit fayl ixtiyoriy": "Файл депозита (необяз.)",
   "Depozit fayl": "Файл депозита",
   "Yillik arxivni birdan yuklash": "Загрузить годовой архив",
   "Fayllar sanasi nomidan olinadi va arxivga qo'shiladi": "Дата файлов определяется из имени",
   "To'lov jadvallarini shakllantirish": "Сформировать таблицы платежей",
   "Ko'rsatkich nomi": "Наименование показателя",
   "Partiya soni": "Кол-во партий",
   "Xodimlar": "Сотрудники",
   "Statistika": "Статистика",
   "ta AWB reytingi": "AWB рейтинг",
   "ta ombor reytingi": "рейтинг складов",
   "ta tovar guruhi": "групп товаров",
   "ta korxona reytingi": "рейтинг предприятий",
   "Jami BKO": "Всего БКО",
   "Jami yozuvlar": "Всего записей",
   "korxona reytingi": "рейтинг предприятий",
   "Yangi fayl yuklash": "Загрузить новый файл",
   "Oziq-ovqatlar kesimida": "Продовольственные товары в разрезе",
   ...
   "Toshkent-AERO IBK bo'yicha BNRTE nazoratdagi tovarlar, to'lovlar, arxiv, nazoratdan yechilish va tahliliy ko'rsatkichlar yagona dashboardda jamlanadi.": "Товары, платежи, архив, снятие с контроля и аналитические показатели BNRTE Ташкент-AERO IBK объединены в единой панели.",
   "70-74-80: postlar kesimida": "70-74-80: в разрезе постов",
   "Post qatorlari ustiga bosilganda asos deklaratsiyalar ochiladi.": "При нажатии на строку поста открываются исходные декларации.",
   "Jami muddati o'tgan: postlar va rejimlar kesimida": "Итого просрочено: в разрезе постов и режимов",
   "Bojxona postlari": "Таможенные посты", "Jami nazoratdagi": "Итого под контролем",
   "Rejim muddati o'tgan tovarlar": "Просроченные товары по режиму", "Shundan": "Из них",
   "Vaqtincha saqlash IM70": "Временное хранение ИМ70", "Bojxona ombori IM74": "Таможенный склад ИМ74",
   "Tranzit TR80": "Транзит ТР80", "Qiymati,": "Стоимость,", "ming doll.": "тыс. долл.",
   "Partiyadagi": "Доля", "ulushi (%)": "в партии (%)",
   "Jami va 70-74-80": "Итого и 70-74-80", "70-74-80 rejimlar kesimida": "70-74-80 в разрезе режимов",
   "Rejimlar qiymat ulushi": "Доля стоимости по режимам", "Rejimlar to'lov ulushi": "Доля платежей по режимам",
   "Omborlar kesimida": "В разрезе складов", "Omborlar qiymat ulushi": "Доля стоимости по складам",
   "O'z ombor jami": "Итого собственный склад", "O'z ombor 3 oy+": "Собственный склад 3 мес.+",
   "O'z ombor partiya ulushi": "Доля партий собственного склада",
   "Omborlar oboroti (nazoratdan yechilgan)": "Оборот складов (снято с контроля)",
   "Omborlar bo'yicha yuk oqimi tendensiyasi": "Тенденция грузопотока по складам",
   "Muddatlar kesimida": "В разрезе сроков", "Muddatlar partiya ulushi": "Доля партий по срокам",
   "Saqlanish sabablari qiymat ulushi": "Доля стоимости по причинам хранения",
   "TOP 20 korxona (qiymat)": "ТОП-20 предприятий (по стоимости)",
   "TOP 20 korxona (depozit mablag'lari)": "ТОП-20 предприятий (по депозиту)",
   "Qiymat ulushi": "Доля стоимости", "Korxonalar bo'yicha davriy tendensiya": "Периодическая тенденция по предприятиям",
   "Transport kesimida korxonalar — yuk oqimi tendensiyasi": "Предприятия в разрезе транспорта — тенденция грузопотока",
   "Transport turi bo'yicha davriy tendensiya": "Периодическая тенденция по видам транспорта",
   "Muddati o'tgan postlar kesimida": "Просроченные в разрезе постов",
   "Muddati o'tgan jamlanma": "Сводная — просроченные", "Muddati o'tgan korxonalar": "Просроченные предприятия",
   "Nazoratdan yechilishi": "Снятие с контроля", "kun ichida yechilgan": "дней снято с контроля",
   "1 oy ichida yechilgan": "снято с контроля за 1 месяц",
   "sanasidagi asos fayl arxivda yo'q.": "исходный файл за эту дату отсутствует в архиве.",
   "Boshlang'ich sana (eskiroq) va yakuniy sana (yangiroq) tanlang, so'ng Hisoblash tugmasini bosing.": "Выберите начальную (более раннюю) и конечную (более позднюю) дату, затем нажмите «Рассчитать».",
   "Tovarlar bo'yicha davriy tendensiya": "Периодическая тенденция по товарам", "Tovarlar guruhlari": "Группы товаров",
   "Partiya bo'yicha ulushi": "Доля по партиям", "Vazn bo'yicha ulushi": "Доля по весу",
   "Nazoratga qo'yilgan postlar va transport turlari": "Посты и виды транспорта под контролем",
   "Barcha qiymatlar ming AQSh dollarida (ming $) berilgan. Post ulushi umumiy qiymat asosida hisoblanadi.": "Все значения указаны в тыс. долларов США (тыс. $). Доля поста рассчитывается на основе общей стоимости.",
   "muddati o'tgan": "просрочено", "sug'urtasi o'tgan": "страховка просрочена",
   "so'm undirilgan": "сум взыскано", "3 oy+": "3 мес.+", "ming $": "тыс. $",
   "Iste'mol muddati o'tgan": "Истёкший срок годности",
   "BNRTE — jami partiya": "БНРТЭ — всего партий",
   "To'lovlar — jami summa (so'm)": "Платежи — общая сумма (сум)",
   "Yangilangan:": "Обновлено:", "Eng ko'p partiya qayd etilgan post:": "Пост с наибольшим числом партий:",
   "Kutilayotgan to'lov (mln so'm)": "Ожидаемый платёж (млн. сум)",
   "partiya": "партия", "vazn": "вес", "qiymat": "стоимость",
   "Transport turi bo'yicha tahlil": "Анализ по видам транспорта",
   "Shundan o'z omborda": "Из них в собственном складе"};
  _trWalk(M);
}
adminPanel=function(){
  let tab=window._adminTab||'users';
  return `<div class=stack><div class="panel" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px"><div><h2 style="margin:0">⚙️ Server boshqaruvi</h2><div class=muted style="font-size:12px;margin-top:3px">Python serverini qayta ishga tushirish — yangi kod kuchga kiradi</div></div><div style="display:flex;gap:8px;align-items:center"><span id="srvRestartMsg" class=muted style="font-size:12px"></span><button onclick="adminRestart(this)" style="background:#fef2f2;color:#b42318;border-color:#fca5a5">🔄 Serverni restart qilish</button></div></div>
  <div class="panel" style="padding:10px 14px"><div style="display:flex;gap:6px;flex-wrap:wrap">
    <button class="${tab==='users'?'btn':'btn light'}" onclick="window._adminTab='users';$('adminUsersView').style.display='';$('adminStatView').style.display='none'">Xodimlar</button>
    <button class="${tab==='stat'?'btn':'btn light'}" onclick="window._adminTab='stat';$('adminUsersView').style.display='none';$('adminStatView').style.display='';loadStatistika()">📊 Statistika</button>
  </div></div>
  <div id="adminUsersView" style="display:${tab==='users'?'':'none'}"><div class=admin-layout><div class="admin-card"><h2>👤 Hodim qo'shish yoki tahrirlash</h2><form id=userForm class=admin-form><input type=hidden name=edit_mode value=""><label>Login</label><input name=user required><label>Yangi parol</label><div class="pass-wrap"><input name=password type=password placeholder="Tahrirda bo'sh qoldirish mumkin"><button class="eye-btn" type="button" onclick="let p=this.previousElementSibling;p.type=p.type==='password'?'text':'password'" title="Ko'rsatish/yashirish">&#128065;</button></div><label>F.I.Sh.</label><input name=full_name><label>Lavozim</label><input name=position><label>Telefon</label><input name=phone><label>Email</label><input name=email type=email placeholder="parolni tiklash uchun"><label>Post kodi</label><input name=post_code placeholder="00 = IBK, aks holda post kodi"><label>Rol</label><select name=role><option value=foydalanuvchi>Foydalanuvchi</option><option value=inspektor>Inspektor</option><option value=rahbar>Rahbar</option><option value=admin>Admin</option></select><label>Til</label><select name=lang><option value=uz>O'zbek lotin</option><option value=uzc>O'zbek kirill</option><option value=ru>Rus tili</option></select><div class=perm-grid><label><input type=checkbox name=perm_view checked> Ko'rish</label><label><input type=checkbox name=perm_upload> Yuklash</label><label><input type=checkbox name=perm_export> Eksport</label><label><input type=checkbox name=perm_release> Yechish</label></div><div class=excel-actions><button>Saqlash</button><button type=button class=light onclick="resetUserForm()">Tozalash</button></div></form></div><div class="admin-card"><h2>Hodimlar ro'yxati</h2><div id=users></div></div></div>${siteMapPanel()}</div>
  <div id="adminStatView" style="display:${tab==='stat'?'':'none'}"><div id="adminView"><div class=muted>Yuklanmoqda...</div></div></div>
  </div>`
}
async function adminRestart(btn){
  if(!confirm('Python server qayta ishga tushiriladi (2-3 soniya o\'chib yonadi). Davom etasizmi?'))return;
  setBusy(btn,true,'Restart...');
  const msg=$('srvRestartMsg');
  try{
    await api('/api/admin/restart',{method:'GET'});
    if(msg)msg.textContent='Qayta ishga tushmoqda...';
    setTimeout(async()=>{
      for(let i=0;i<10;i++){
        await new Promise(r=>setTimeout(r,1500));
        try{await api('/api/server_info');if(msg)msg.textContent='✓ Server ishga tushdi';setBusy(btn,false);return}catch(e){}
      }
      if(msg)msg.textContent='Server javob bermayapti';setBusy(btn,false);
    },2000);
  }catch(e){if(msg)msg.textContent='Xatolik: '+e.message;setBusy(btn,false);}
}
...
bindUserForm=function(){let f=$("userForm");if(!f)return;f.onsubmit=async e=>{e.preventDefault();let perms=[];["view","upload","export","release"].forEach(p=>{if(f[`perm_${p}`]?.checked)perms.push(p)});let body={user:f.user.value,pass:f.password.value,full_name:f.full_name.value,position:f.position.value,phone:f.phone.value,email:f.email.value,post_code:f.post_code.value,role:f.role.value,lang:f.lang.value,perms,enabled:true};await api(f.edit_mode.value?"/api/users/update":"/api/users",{method:"POST",body:JSON.stringify(body)});resetUserForm();loadUsers()}}
window.editUser=function(login){api("/api/users").then(j=>{let u=(j.users||[]).find(x=>x.user===login),f=$("userForm");if(!u||!f)return;f.edit_mode.value="1";f.user.value=u.user;f.user.disabled=true;f.password.value="";f.full_name.value=u.full_name||"";f.position.value=u.position||"";f.phone.value=u.phone||"";f.email.value=u.email||"";f.post_code.value=u.post_code||"";f.role.value=u.role||"foydalanuvchi";f.lang.value=u.lang||"uz";["view","upload","export","release"].forEach(p=>f[`perm_${p}`].checked=(u.perms||[]).includes(p));window.scrollTo({top:0,behavior:"smooth"})})}
...
const _tabSnaps={};
function captureTabSnap(tab){
  const v=$('view');if(!v||!v.innerHTML.trim())return;
  _tabSnaps[tab]=v.innerHTML;
  const smGrid=$('smMapGrid');if(!smGrid)return;
  const inner=smGrid.querySelector(`[data-sm-id="${tab}"] .sm-thumb-inner`);
  if(inner)inner.innerHTML=v.innerHTML;
}
...
function siteMapPanel(){
  ...
  function tabCard(id,modId){
    ...
    const snap=_tabSnaps[id]||'';
    const thumbHtml=snap
      ?`<div class="sm-thumb"><div class="sm-thumb-inner">${snap}</div></div>`
      :`<div class="sm-thumb"><div class="sm-thumb-empty">${m.icon}</div></div>`;
    return `<div class="sm-card${isH?' sm-hid':''}" data-sm-id="${id}" data-sm-mod="${modId}"
      ...
      <button class=sm-card-eye onclick="event.stopPropagation();smCardEye('${id}',this)" title="Ko'rish/Yashirish">${isH?'👁':'🙈'}</button>
      ${thumbHtml}
...
// ─────────────────────────────────────────────────────────────────────────────
const renderClean=render;render=function(){renderClean();cleanTopActions();if(LANG==="ru")translateRuPage();else if(LANG==="uzc")translateUzcPage();applyUIConfig()}
let _trTimer=null;
const _trObs=(function(){const obs=new MutationObserver(()=>{if(LANG==='uz')return;clearTimeout(_trTimer);_trTimer=setTimeout(()=>{obs.disconnect();if(LANG==='ru')translateRuPage();else if(LANG==='uzc')translateUzcPage();const a=$('app');if(a)obs.observe(a,{childList:true,subtree:true});},120);});return obs;})();
(function(){const a=$('app');if(a)_trObs.observe(a,{childList:true,subtree:true});})();
...
function showUploadProgress(label,pct){
  ...
  if(gb&&gl&&gp){gb.style.display='';gl.textContent=label;gl.style.color=c;gp.style.width=Math.max(3,pct)+'%';gp.style.background=c;}
}
  ...
  try{
    ...
    async function uploadOne(i){
      ...
      for(let attempt=0;attempt<MAX_RETRY;attempt++){
        ...
        try{
          if(_signal.aborted)throw new Error('Yuklash bekor qilindi');
...
function handleUnifiedDrop(files){
  ...
  list.innerHTML='<div style="margin:6px 0;font-size:13px;color:#6b7280;font-weight:600">Aniqlangan fayllar:</div>'+_unifiedFiles.map((f,i)=>{
    ...
    return `<div style="display:flex;align-items:center;gap:8px;padding:7px 10px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:7px;margin-bottom:5px">
      ...
      <span style="font-size:13px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${esc(f.name)}" translate="no">${esc(f.name)}</span>
...
async function runUnifiedUpload(btn){
  ...
  for(let i=0;i<_unifiedFiles.length;i++){
    ...
    const t=types[i];
    if(prog)prog.innerHTML=`<div style="font-size:13px;color:#1d72b8;font-weight:600">${i+1}/${_unifiedFiles.length}: <span translate="no">${esc(f.name)}</span> (${FILE_TYPE_LABELS[t]}) yuklanyapti...</div>`;
...
async function ucUploadDepozit(btn){
  ...
  let repSel=$('ucDepozitReport');
  let reportId=repSel?.value||(DATA?DATA.id:'');
  if(!reportId){if(st)st.innerHTML='<span style="color:#b91c1c;font-weight:600">⚠ Avval BNRTE asos faylini yuklang va hisoblash tugashini kuting</span>';return}
  ...
  try{
    ...
    let j=await api('/api/chunk_finalize_deposit',{method:'POST',body:JSON.stringify({...info,report_id:reportId})});
...
async function ucUploadVaqtincha(btn){
  let src=$('ucVaqtinchaFile'),st=$('ucVaqtinchaStatus');
  if(!src?.files?.length){if(st)st.textContent='Fayl tanlang';return}
  setBusy(btn,true,'Yuklanmoqda');if(st)st.textContent='Yuklanyapti...';
  try{
    const info=await chunkedUpload(src.files[0],p=>{if(st)st.textContent=`Yuklanyapti: ${Math.round(p*100)}%`;});
    if(st)st.textContent='Qayta ishlanmoqda...';
    let j=await api('/api/chunk_finalize_vaqtincha',{method:'POST',body:JSON.stringify(info)});
    if(j.job_id){pollVaqtincha(j.job_id,st,btn);}
...
<div style="margin-top:10px;display:flex;gap:8px"><button id="unifiedBtn" onclick="runUnifiedUpload(this)" style="display:none">🚀 Yuklash</button></div></div>`;
  let mainBody=`<div class="panel uc-card"><div class=uc-header><h2>${ucIcon('box')}BNRTE — Nazoratdagi tovarlar</h2>${bnrteSt}</div><div class=uc-body><div class=uc-field><label>Asos fayl (xls/xlsx)</label><input type=file id="ucBnrteSource" accept=".xls,.xlsx,.html,.htm"></div><div class=uc-btns><button onclick="ucUploadBnrte(this)">Yuklash</button><button class="btn light" onclick="refreshCurrentReport(this)">🔄 Qayta hisoblash</button></div></div><div class=muted id="ucBnrteStatus" style="margin-top:8px"></div></div>
<div class="panel uc-card"><div class=uc-header><h2>${ucIcon('refresh')}Depozit fayl — alohida yuklash</h2>${depSt}</div><div class=uc-body><div class=uc-field><label>Qaysi hisobotga biriktirilsin</label><select id="ucDepozitReport">${reportOptions()}</select></div><div class=uc-field><label>Depozit fayl (xlsx)</label><input type=file id="ucDepozitFile" accept=".xlsx"></div><div class=uc-btns><button onclick="ucUploadDepozit(this)">Yuklash</button></div></div><div class=muted style="margin-top:6px;font-size:12px">Tanlangan hisobotga depozit ma'lumotlarini biriktiradi va qayta hisoblab chiqadi.</div><div id="ucDepozitStatus" style="margin-top:6px;font-size:13px"></div></div>
<div class="panel uc-card"><div class=uc-header><h2>${ucIcon('receipt')}To'lovlar jadvallari</h2>${tolovSt}</div><div class=uc-body><div class=uc-field><label>To'lov baza fayli (xlsx)</label><input type=file id="ucTolovSource" accept=".xlsx,.xls,.zip,.rar"></div><div class=uc-btns><button onclick="ucUploadTolov(this)">Yuklash</button><button class="btn light" onclick="ucRecalcTolov(this)">🔄 Qayta hisoblash</button></div></div><div class=muted id="ucTolovStatus" style="margin-top:8px"></div></div>
<div class="panel uc-card"><div class=uc-header><h2>${ucIcon('building')}Omborlar reestri</h2>${wrSt}</div><div class=uc-body><div class=uc-field><label>Reestri fayli (omborlarReestri*.xlsx)</label><input type=file id="ucWrFile" accept=".xlsx,.xls,.zip,.rar"></div><div class=uc-btns><button onclick="ucUploadWarehouse(this)">Yuklash</button><button class="btn light" onclick="ucRecalcWarehouse(this)">🔄 Qayta hisoblash</button></div></div><div class=muted id="ucWrStatus" style="margin-top:8px"></div></div>`;
  let extraBody=`<div class="panel uc-card" style="border-left-color:#0ea5e9"><div class=uc-header><h2>${ucIcon('plane')}AVIA AWB</h2><div style="display:flex;gap:6px;flex-wrap:wrap">${aviaSt} ${aviaDbSt}</div></div><div class=uc-note>AWB Excel (Yuklarni qabul qilish.xlsx) → AWB ro'yxati, joylar, vazn. Qiymat (ming $) BNRTE asos faylidan olinadi — yangilash uchun BNRTE → Qayta hisoblash.</div><div class=uc-body style="margin-top:12px"><div class=uc-field><label>AWB Excel (Yuklarni qabul qilish*.xlsx)</label><input type=file id="ucAviaFile" accept=".xlsx,.xls,.zip,.rar"></div><div class=uc-btns><button onclick="ucUploadAvia(this)">Yuklash</button><button class="btn light" onclick="ucRecalcAvia(this)">🔄 Qayta hisoblash</button></div></div><div class=muted id="ucAviaStatus" style="margin-top:8px"></div></div>
<div class="panel uc-card" style="border-left-color:#6366f1"><div class=uc-header><h2>${ucIcon('document')}Vaqtincha IM42/EK12</h2>${vaqSt}</div><div class=uc-note>IM42/EK12 vaqtincha deklaratsiyalar ro'yxati (HTML-format .xls fayl).</div><div class=uc-body style="margin-top:12px"><div class=uc-field><label>Asos fayl (IM42/EK12 .xls)</label><input type=file id="ucVaqtinchaFile" accept=".xls,.xlsx,.html,.htm"></div><div class=uc-btns><button onclick="ucUploadVaqtincha(this)">Yuklash</button></div></div><div class=muted id="ucVaqtinchaStatus" style="margin-top:8px"></div></div>
<div class="panel uc-card" style="border-left-color:#7c3aed"><div class=uc-header><h2>${ucIcon('truck')}Avtotaqsimot</h2></div><div class=uc-body><div class=uc-field><label>Avtotaqsimot fayli (xls/htm)</label><input type=file id="ucAvtoFile" accept=".xlsx,.xls,.htm,.html,.zip,.rar"></div><div class=uc-btns><button onclick="ucUploadGeneric(this,'ucAvtoFile','ucAvtoSt','/api/upload_avtotaqsimot','Avtotaqsimot')">Yuklash</button></div><div class=muted id="ucAvtoSt" style="margin-top:8px"></div></div>`;
  let bkoBody=`<div class="panel uc-card" style="border-left-color:#059669"><div class=uc-header><h2>${ucIcon('document')}BKO</h2></div><div class=uc-body><div class=uc-field><label>BKO — postlar kesimida</label><input type=file id="ucBkoPostFile" accept=".xlsx,.xls,.zip,.rar"></div><div class=uc-btns><button onclick="ucUploadGeneric(this,'ucBkoPostFile','ucBkoPostSt','/api/upload_bko_postlar','BKO postlar')">Yuklash</button></div><div class=muted id="ucBkoPostSt" style="margin-top:4px"></div><div class=uc-field style="margin-top:10px"><label>BKO — xodimlar kesimida</label><input type=file id="ucBkoXodFile" accept=".xlsx,.xls,.zip,.rar"></div><div class=uc-btns><button onclick="ucUploadGeneric(this,'ucBkoXodFile','ucBkoXodSt','/api/upload_bko_xodimlar','BKO xodimlar')">Yuklash</button></div><div class=muted id="ucBkoXodSt" style="margin-top:4px"></div><div class=uc-field style="margin-top:10px"><label>BKO — rasmiylashtirilgan</label><input type=file id="ucBkoRasmFile" accept=".xlsx,.xls,.zip,.rar"></div><div class=uc-btns><button onclick="ucUploadGeneric(this,'ucBkoRasmFile','ucBkoRasmSt','/api/upload_bko_rasmiy','BKO rasmiylashtirilgan')">Yuklash</button></div><div class=muted id="ucBkoRasmSt" style="margin-top:4px"></div></div></div>
<div class="panel uc-card" style="border-left-color:#d97706"><div class=uc-header><h2>${ucIcon('search')}Ko'riklar</h2></div><div class=uc-body><div class=uc-field><label>Ko'riklar va holatlar</label><input type=file id="ucKorikHolFile" accept=".xlsx,.xls,.zip,.rar"></div><div class=uc-btns><button onclick="ucUploadGeneric(this,'ucKorikHolFile','ucKorikHolSt','/api/upload_korik_holatlar','Ko\\'riklar va holatlar')">Yuklash</button></div><div class=muted id="ucKorikHolSt" style="margin-top:4px"></div><div class=uc-field style="margin-top:10px"><label>Ko'rik — sarflangan vaqt</label><input type=file id="ucKorikVaqtFile" accept=".xlsx,.xls,.zip,.rar"></div><div class=uc-btns><button onclick="ucUploadGeneric(this,'ucKorikVaqtFile','ucKorikVaqtSt','/api/upload_korik_vaqt','Ko\\'rik sarflangan vaqt')">Yuklash</button></div><div class=muted id="ucKorikVaqtSt" style="margin-top:4px"></div></div></div>`;
  let archiveBody=`<div class="panel uc-card"><div class=uc-header><h2>${ucIcon('archive')}Yillik arxivni birdan yuklash</h2></div><div class=uc-note>Bir vaqtning o'zida bir nechta asos fayl tanlanadi. Fayllar navbatga qo'shiladi va server mustaqil qayta hisoblab chiqadi — admin kutib o'tirmasligi kerak. Fayl sanasi nom ichidan avtomatik aniqlanadi.</div><form id="bulkUpload"><div class=uc-body><div class=uc-field><label>Asos fayllar (bir nechta)</label><input name="sources" type="file" accept=".xls,.xlsx,.html,.htm" multiple required></div><div class=uc-btns><button>Hammasini yuklash</button></div></div></form><div id=bulkResult class=muted style="margin-top:8px">Fayllar tanlanmagan.</div></div>`;
...
${grp('quick','folder',"Tezkor yuklash",quickBody)}
${grp('main','box',"Asosiy hisobotlar",mainBody)}
${grp('extra','plane',"Qo'shimcha reestrlar",extraBody)}
${grp('bko','document',"BKO hisobotlari",bkoBody)}
${grp('archive','archive',"Yillik arxiv",archiveBody)}
</div>`;
}
...
function toggleRegimeYearPost(post){if(EXPANDED_POST.has(post))EXPANDED_POST.delete(post);else EXPANDED_POST.add(post);render();}
function toggleRegimeYearRegime(key){if(EXPANDED_REGIME.has(key))EXPANDED_REGIME.delete(key);else EXPANDED_REGIME.add(key);render();}
function interactiveRegimeYearTable(){
  let rows=DATA.regime_year_post||[],posts=[...new Set(rows.map(r=>r.post))];
  let body=posts.map(p=>{
    let pr=rows.filter(r=>r.post===p);
    let pt={partiya:0,vazn:0,qiymat:0,tolov:0};
    pr.forEach(x=>{pt.partiya+=(+x.partiya||0);pt.vazn+=(+x.vazn||0);pt.qiymat+=(+x.qiymat||0);pt.tolov+=(+x.tolov||0);});
    let postOpen=EXPANDED_POST.has(p);
    let pEsc=esc(p).replaceAll("'","&#39;");
    let out=[`<tr class="merged-row" style="cursor:pointer" onclick="toggleRegimeYearPost('${pEsc}')"><td style="text-align:left">${postOpen?'▾':'▸'} ${esc(p)}</td><td class=num>${fmtI(pt.partiya)}</td><td class=num>${fmtN(pt.vazn)}</td><td class=num>${fmtN(pt.qiymat)}</td><td class=num>${fmtN(pt.tolov)}</td></tr>`];
    if(postOpen)["IM70","IM74","TR80"].forEach(reg=>{
      let rr=pr.filter(x=>x.rejim===reg);
      if(!rr.length)return;
      let regKey=p+'|'+reg, regOpen=EXPANDED_REGIME.has(regKey), regKeyEsc=esc(regKey).replaceAll("'","&#39;");
      let rt={partiya:rr.reduce((a,x)=>a+(+x.partiya||0),0),vazn:rr.reduce((a,x)=>a+(+x.vazn||0),0),qiymat:rr.reduce((a,x)=>a+(+x.qiymat||0),0),tolov:rr.reduce((a,x)=>a+(+x.tolov||0),0)};
      out.push(`<tr class="sub-total" style="cursor:pointer" onclick="toggleRegimeYearRegime('${regKeyEsc}')"><td style="text-align:left;padding-left:24px">${regOpen?'▾':'▸'} ${esc(reg)}</td><td class=num>${fmtI(rt.partiya)}</td><td class=num>${fmtN(rt.vazn)}</td><td class=num>${fmtN(rt.qiymat)}</td><td class=num>${fmtN(rt.tolov)}</td></tr>`);
      if(regOpen)rr.forEach(x=>out.push(`<tr onclick='detail(${JSON.stringify(x.key||{}).replaceAll("'","&#39;")})'><td style="text-align:left;padding-left:42px">${esc(x.yil||'-')} yil</td><td class=num>${fmtI(x.partiya)}</td><td class=num>${fmtN(x.vazn)}</td><td class=num>${fmtN(x.qiymat)}</td><td class=num>${fmtN(x.tolov)}</td></tr>`));
    });
    return out.join("");
  }).join("");
  let total={partiya:rows.reduce((a,x)=>a+(+x.partiya||0),0),vazn:rows.reduce((a,x)=>a+(+x.vazn||0),0),qiymat:rows.reduce((a,x)=>a+(+x.qiymat||0),0),tolov:rows.reduce((a,x)=>a+(+x.tolov||0),0)};
  let totalRow=`<tr class="grand-total"><td style="text-align:left">IBK bo'yicha Jami</td><td class=num>${fmtI(total.partiya)}</td><td class=num>${fmtN(total.vazn)}</td><td class=num>${fmtN(total.qiymat)}</td><td class=num>${fmtN(total.tolov)}</td></tr>`;
  return `<table class="sample-like regime-year-table"><colgroup><col style="width:230px"><col style="width:90px"><col style="width:115px"><col style="width:126px"><col style="width:132px"></colgroup><thead><tr><th>Ko'rsatkich nomi</th><th>Partiya soni</th><th>Vazni<br>(tn.)</th><th>Qiymati<br>(ming $)</th><th>To'lov<br>(mln.so'm)</th></tr></thead><tbody>${totalRow}${body}</tbody></table>`;
}
function interactiveFoodTable(){
  let rows=foodRows().map((r,i)=>Object.assign({rn:i?i:"Jami:"},r));
  let body=rows.map(r=>{
    let ulushPct=Math.max(0,Math.min(100,+r.ulush||0));
    let eskPct=(+r.qiymat||0)>0?Math.max(0,Math.min(100,(+r.over_qiymat||0)/(+r.qiymat||1)*100)):0;
    let eskColor=eskPct>55?'#b8460e':'#d95f02';
    return `<tr onclick='detail(${JSON.stringify(r.key||{}).replaceAll("'","&#39;")})'><td class=num>${esc(r.rn)}</td><td class=text title="${esc(r.name)}">${esc(r.name)}</td><td class=num>${fmtN(r.vazn)}</td><td class=num>${fmtN(r.qiymat)}</td><td class=num>${fmtN(r.over_vazn)}</td><td class=num>${fmtN(r.over_qiymat)}</td><td class=num><div style="display:flex;align-items:center;gap:6px;justify-content:flex-end"><div style="flex:1;max-width:56px;background:#e3edf6;border-radius:4px;height:6px;overflow:hidden"><div style="width:${ulushPct}%;height:100%;background:linear-gradient(90deg,#1f6fb8,#35c1c9)"></div></div><span>${ulushPct.toFixed(1)}%</span></div></td><td class=num><div style="display:flex;align-items:center;gap:6px;justify-content:flex-end"><div style="flex:1;max-width:56px;background:#f7e9df;border-radius:4px;height:6px;overflow:hidden"><div style="width:${eskPct}%;height:100%;background:linear-gradient(90deg,#d95f02,#f5a742)"></div></div><span style="color:${eskColor};font-weight:700">${eskPct.toFixed(1)}%</span></div></td></tr>`;
  }).join("");
  return `<table class="sample-like food-sample"><colgroup><col style="width:36px"><col style="width:auto"><col style="width:86px"><col style="width:110px"><col style="width:86px"><col style="width:110px"><col style="width:100px"><col style="width:100px"></colgroup><thead><tr><th rowspan=2>№</th><th rowspan=2>Tovarlar turi</th><th rowspan=2>Vazni,<br>tn</th><th rowspan=2>Qiymati,<br>ming AQSH doll.</th><th colspan=2>Shundan, 3 oydan oshganlari</th><th rowspan=2>Qiymatdagi<br>ulushi (%)</th><th rowspan=2>Eskirish (%)</th></tr><tr><th>Vazni,<br>tn</th><th>Qiymati,<br>ming AQSH doll.</th></tr></thead><tbody>${body}</tbody></table>`;
}
const renderSampleTables=render;render=function(){renderSampleTables();let v=$("view");if(!v||!DATA)return;if(TAB==="rejim")v.innerHTML=`<div class=stack><div class=panel><h2>70-74-80: post, rejim va yillar kesimida</h2>${interactiveRegimeYearTable()}</div><div class=panel><h2>Rejimlar qiymat bo'yicha ulushi</h2>${bars(DATA.regimes||[],"rejim","qiymat",fmtN)}</div></div>`;if(TAB==="food")v.innerHTML=`<div class=panel><h2>Oziq-ovqatlar kesimida ${xls("food")}</h2>${interactiveFoodTable()}</div>`}
...
function vaqtinchaPanel(){
  ...
  // Tovarlar kesimida (goods breakdown)
  const goodsMap={};rows.forEach(r=>{const k=(r.hs_code||'').slice(0,4)||'—';if(!goodsMap[k])goodsMap[k]={hs:k,goods:r.goods||'',count:0,weight:0,value:0,expired:0};goodsMap[k].count++;goodsMap[k].weight+=r.weight_net||0;goodsMap[k].value+=r.value_usd||0;if(r.expired)goodsMap[k].expired++;});
  const goodsRows=Object.values(goodsMap).sort((a,b)=>b.value-a.value);
  const goodsTbl=`<div style="overflow-x:auto;max-height:420px;overflow-y:auto"><table style="width:100%;border-collapse:collapse;font-size:12px">
    <thead><tr style="background:#f0f4fb;border-bottom:2px solid var(--line)">
      <th style="padding:7px 8px;text-align:left;font-weight:700;font-size:12px;width:70px">TN VED</th>
      <th style="padding:7px 8px;text-align:left;font-weight:700;font-size:12px">Tovar nomi (namuna)</th>
      <th style="padding:7px 8px;text-align:right;font-weight:700;font-size:12px;width:70px">Soni</th>
      <th style="padding:7px 8px;text-align:right;font-weight:700;font-size:12px;width:100px">Netto (kg)</th>
      <th style="padding:7px 8px;text-align:right;font-weight:700;font-size:12px;width:100px">Qiymat ($)</th>
      <th style="padding:7px 8px;text-align:right;font-weight:700;font-size:12px;width:80px;color:#dc2626">Muddati o'tgan</th>
    </tr></thead><tbody>${goodsRows.map(g=>`<tr style="border-bottom:1px solid var(--line)${g.expired>0?';background:#fffbf0':''}">
      <td style="padding:6px 8px;font-family:monospace;font-size:11px">${esc(g.hs)}</td>
      <td style="padding:6px 8px;font-size:11px;max-width:280px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${esc(g.goods)}">${esc(g.goods.substring(0,80))}</td>
      <td style="padding:6px 8px;text-align:right;font-size:12px">${fmtI(g.count)}</td>
      <td style="padding:6px 8px;text-align:right;font-size:12px">${fmtN(g.weight)}</td>
      <td style="padding:6px 8px;text-align:right;font-size:12px;font-weight:600">${fmtN(g.value)}</td>
      <td style="padding:6px 8px;text-align:right;font-size:12px;font-weight:600;color:${g.expired>0?'#dc2626':'#6b7280'}">${g.expired>0?g.expired:'—'}</td>
    </tr>`).join('')}</tbody></table></div>`;
  return `<div class=stack>
    <div class=panel><h2>🔄 Vaqtincha IM42/EK12 ${dateFreshnessBadge(vd.report_date)}</h2>
    ...
    <div class="panel wide"><h2>Tovarlar kesimida (TN VED bo'yicha)</h2><p class=muted style="margin-bottom:8px">Joriy filtr bo'yicha tovarlar guruhlangan (TN VED dastlabki 4 ta raqam). Sariq = muddati o'tgan bor.</p>${goodsTbl}</div>
  ...
  const timer=setInterval(async()=>{
    try{
      ...
      if(j.status==='tayyor'){
        ...
        if(st)st.textContent=`✓ Tayyor: ${(VAQTINCHA_DATA.items||[]).length} ta yozuv`;
        if(btn)setBusy(btn,false);
        if(TAB==='vaqtincha')render();
      }else if(j.status==='xatolik'){clearInterval(timer);console.error('Vaqtincha xatolik:',j.error);if(st)st.textContent='Xatolik: '+(j.error||'');if(btn)setBusy(btn,false);}
    }catch(e){clearInterval(timer);if(btn)setBusy(btn,false);}
  },1500);
}
...
function renderAviaContent(j){
  ...
  let countryHtml=`<div class=panel><h2>Davlatlar bo'yicha (vazn, tn) — TOP 15</h2>${bars(countries15,'name','vazn',fmtN,5)}</div>`;
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/analytics":
            if not self.require_admin():
                return
            self.json(get_analytics_summary())
            return
        ...
        if parsed.path == "/api/admin/restart":
            if not self.require_admin():
                return
            self.json({"ok": True, "msg": "Server qayta ishga tushmoqda..."})
        ...
        if parsed.path == "/api/users":
            if not self.require_admin():
                return
            users = load_json(USER_PATH, {})
            self.json({"users": [{"user": u, "role": r.get("role", "user"), "role_label": r.get("role_label", ROLE_LABELS.get(r.get("role", "user"), "Foydalanuvchi")), "post_code": r.get("post_code", ""), "enabled": r.get("enabled", True), "full_name": r.get("full_name", ""), "position": r.get("position", ""), "phone": r.get("phone", ""), "email": r.get("email", ""), "lang": r.get("lang", "uz"), "perms": r.get("perms", [])} for u, r in users.items()]})
            return
        ...
        if parsed.path.startswith("/download/tolov/"):
            ...
            data = path.read_bytes()
            try:
                log_analytics(self.user() or "?", "download", filename=path.name, kind="tolov")
            except Exception:
                pass
            self.send_response(HTTPStatus.OK)
            self.send_header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        ...
        if parsed.path.startswith("/download/"):
            ...
            data = path.read_bytes()
            try:
                log_analytics(self.user() or "?", "download", filename=path.name,
                               kind="report", report_id=report_id)
            except Exception:
                pass
            self.send_response(HTTPStatus.OK)
    ...
    def _do_POST_inner(self):
        ...
        if parsed.path == "/api/analytics/event":
            ...
            try:
                ...
                log_analytics(user, event, **kwargs)
            except Exception:
                pass
            self.json({"ok": True})
            return
        if parsed.path == "/api/login":
            ...
            try:
                log_analytics(data["user"], "login", ip=client_ip)
            except Exception:
                pass
        ...
        if parsed.path == "/api/users":
            ...
            users[user] = {"salt": salt, "password": hash_password(password, salt), "role": role, "role_label": ROLE_LABELS.get(role, "Foydalanuvchi"), "enabled": True, "full_name": data.get("full_name", ""), "position": data.get("position", ""), "phone": data.get("phone", ""), "email": data.get("email", ""), "lang": data.get("lang", "uz"), "post_code": data.get("post_code", ""), "perms": ADMIN_PERMS if role == "admin" else perms}
            save_json(USER_PATH, users)
            self.json({"ok": True})
            return
        ...
        if parsed.path == "/api/users/update":
            ...
            rec.update({
                ...
                "email": data.get("email", rec.get("email", "")),
        ...
        if parsed.path == "/api/chunk_finalize":
            ...
            shutil.rmtree(cd, ignore_errors=True)
            try:
                source = _extract_if_archive(source, tmp)
            except Exception as exc:
                self.json({"error": str(exc)}, HTTPStatus.BAD_REQUEST); return
            deposit = None
        ...
        if parsed.path == "/api/chunk_finalize_wr":
            ...
            shutil.rmtree(cd, ignore_errors=True)
            try:
                dest = _extract_if_archive(dest, DASHBOARD_DIR)
            except Exception as exc:
                self.json({"error": str(exc)}, HTTPStatus.BAD_REQUEST); return
